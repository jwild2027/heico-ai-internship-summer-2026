...
def test_build_script_help_imports_from_repo_root():
    script = Path("scripts/build_trace_net_table_hybrid_retrieval_bridge_v1.py")
...
def test_quality_script_help_imports_from_repo_root():
    script = Path("scripts/check_trace_net_table_hybrid_retrieval_bridge_v1_quality.py")
    result = subprocess.run([sys.executable, str(script), "--help"], capture_output=True, text=True)
    assert result.returncode == 0
