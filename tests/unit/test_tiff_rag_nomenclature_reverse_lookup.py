...
from tiff.rag_answer import answer_question, build_structured_nomenclature_answer
...
def make_clean_catalog_db(path: Path) -> None:
    ...
    conn.executescript(
        ...
        INSERT INTO part_catalog_clean VALUES (
            ...
            '["HOLDER, MAGAZINE"]', CURRENT_TIMESTAMP
        );
        """
    )
    conn.commit()
...
def test_nomenclature_query_resolves_to_part_number_and_mentions(tmp_path: Path) -> None:
    db_path = tmp_path / "search.db"
    make_clean_catalog_db(db_path)

    retrieval = retrieve_rag_context(db_path, "magazine holder", use_embeddings=False, top_k=6)
    source_types = [(s.page_id, s.source_type) for s in retrieval.sources]

    assert ("p1", "nomenclature_catalog_clean") in source_types
    assert ("p1", "part_mentions") not in source_types
    assert ("p0", "part_mentions") in source_types
    assert ("p2", "part_mentions") in source_types
    assert retrieval.sources[0].matched_part_number == "120-37313-001"
    assert retrieval.sources[0].part_nomenclature == "HOLDER, MAGAZINE"


def test_nomenclature_answer_is_deterministic_and_source_grouped(tmp_path: Path) -> None:
    db_path = tmp_path / "search.db"
    make_clean_catalog_db(db_path)

    answer = answer_question(db_path, "Where is magazine holder shown?", use_llm=True, use_embeddings=False)

    assert not answer.used_llm
    assert "HOLDER, MAGAZINE matches part number 120-37313-001." in answer.answer
    assert "Matching catalog source:" in answer.answer
    assert "Additional pages where 120-37313-001 appears:" in answer.answer
    assert "page1056.tif" in answer.answer
    assert "page1021.tif" in answer.answer


def test_structured_nomenclature_answer_returns_none_without_name_match(tmp_path: Path) -> None:
    db_path = tmp_path / "search.db"
    make_clean_catalog_db(db_path)
    retrieval = retrieve_rag_context(db_path, "oxygen bottle", use_embeddings=False)

    assert build_structured_nomenclature_answer("oxygen bottle", retrieval) is None
