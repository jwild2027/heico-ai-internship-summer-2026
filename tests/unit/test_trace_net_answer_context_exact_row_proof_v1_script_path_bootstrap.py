...
def test_check_script_help_runs():
    result = subprocess.run([sys.executable, "scripts/maintenance/ingestion/check_trace_net_answer_context_exact_row_proof_v1_quality.py", "--help"], cwd=Path.cwd(), capture_output=True, text=True)
    assert result.returncode == 0
