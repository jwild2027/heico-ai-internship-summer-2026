...
def test_build_script_help() -> None:
    script = Path("scripts/build/tables/build_trace_net_table_understanding_v1.py")
...
def test_quality_script_help() -> None:
    script = Path("scripts/maintenance/benchmark/check_trace_net_table_understanding_v1_quality.py")
    result = subprocess.run([sys.executable, str(script), "--help"], capture_output=True, text=True)
    assert result.returncode == 0
