...
def test_bridge_precreates_stage_report_directories_for_stage_builders(tmp_path, monkeypatch):
    kernel = tmp_path / "kernel.json"
    kernel.write_text(json.dumps({"quality_status": "PASS"}), encoding="utf-8")

    def write_without_mkdir(path: Path, payload: dict) -> dict:
        assert path.parent.exists(), f"stage directory was not precreated: {path.parent}"
        path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
        return payload

    def fake_query_planner(*, kernel_path, output_dir, questions):
        return write_without_mkdir(
    ...
    def fake_self_rag(*, context_pack_path, output_dir, min_high_signal_capsules, min_evidence_strength_score):
        ...
        return write_without_mkdir(
            ...
            {
                "quality_status": "PASS",
                "summary": {"self_rag_record_count": 1, "ready_for_gemma_draft_count": 1, "crag_retry_required_count": 0},
                "records": [{"answer_permission": False}],
            },
        )
    ...
    def fake_crag(*, self_rag_report_path, output_dir):
        ...
        return write_without_mkdir(
            ...
            {"quality_status": "PASS", "summary": {"crag_retry_plan_count": 0}, "records": []},
        )
    ...
    payload = bridge.build_webui_self_rag_crag_bridge(
        question="Find part number 120-29073-001",
        kernel_path=kernel,
        output_dir=tmp_path / "clean_bridge_output",
    )

    assert payload["quality_status"] == "PASS"
    assert payload["tool_statuses"]["context_pack_blueprint"] == "used"
    assert payload["tool_statuses"]["crag_retry"] == "skipped_not_needed"
