...
def test_build_script_help_runs():
    script = Path("scripts/build_trace_net_element_graph_attachment_plan_v1.py")
...
def test_quality_script_help_runs():
    script = Path("scripts/check_trace_net_element_graph_attachment_plan_v1_quality.py")
    result = subprocess.run([sys.executable, str(script), "--help"], text=True, capture_output=True)
    assert result.returncode == 0
