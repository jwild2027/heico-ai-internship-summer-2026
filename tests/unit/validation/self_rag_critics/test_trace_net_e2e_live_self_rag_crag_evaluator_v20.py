...
def test_v20_reads_v19_evidence_box_items_shape():
    from tiff.trace_net_e2e_live_self_rag_crag_evaluator_v20 import evaluate_pack

    pack = {
        "context_pack_id": "context_pack_v19_regression",
        "user_query": "Find part number 120-36834-509",
        "evidence_box": {
            ...
            "item_count": 2,
            "items": [
                {"field_name": "covered_part_number", "value": "120-36834-509", "page_id": "p1"},
                {"field_name": "covered_part_number", "value": "120-36833-001", "page_id": "p1"},
            ],
        },
        "guidance_box": {
            "graph_guidance": [{"authority": "guidance_only", "proof_authority": False}],
            "v2_summary_guidance": [{"authority": "guidance_only", "proof_authority": False}],
            "graph_authority": "guidance_only",
            "summary_authority": "guidance_only",
        },
        "aggregation_box": {
    ...
    rec = evaluate_pack(pack, 0)
    assert rec["source_truth_evidence_count"] == 2
    assert rec["has_source_truth_evidence"] is True
    assert rec["ready_for_llm_prompt"] is True
    assert rec["self_rag_status"] == "CONTEXT_READY_WITH_CAP_DISCLOSURE"
