...
def test_build_script_help_runs_directly():
    result = subprocess.run(
        [sys.executable, "scripts/build_trace_net_artifact_dirty_planner_v1.py", "--help"],
        text=True,
        capture_output=True,
        check=False,
    )
...
def test_quality_script_help_runs_directly():
    result = subprocess.run(
        [sys.executable, "scripts/check_trace_net_artifact_dirty_planner_v1_quality.py", "--help"],
        text=True,
        capture_output=True,
        check=False,
    )
