...
from tiff.trace_net_e2e_image_visual_observer_route_v34_3 import (
    MODEL_ID,
...
def test_build_report_standard_demo_passes_quality(tmp_path: Path) -> None:
    ...
    assert report["sample_query_count"] == 5
    assert report["sample_success_count"] == 5
    assert report["visual_package_count"] == 5
    assert report["image_quality_card_count"] == 5
    assert report["visual_observation_card_count"] == 5
    assert report["llava_observer_card_count"] == 5
    assert report["guidance_only_visual_card_count"] == 5
...
def test_evaluate_quality_thresholds_pass(tmp_path: Path) -> None:
    ...
    checks = evaluate_quality(
        report,
        min_sample_queries=5,
        min_sample_successes=5,
        min_visual_packages=5,
        min_image_quality_cards=5,
        min_visual_observation_cards=5,
        min_llava_observer_cards=5,
        min_guidance_only_visual_cards=5,
        min_self_rag_samples=5,
        min_crag_samples=5,
...
def test_model_id_is_v34() -> None:
    assert MODEL_ID == "trace-net-e2e-technical-drawing-geometry-llava-v34-3"
...
def test_diagram_draft_request_returns_mermaid_and_json_card() -> None:
    ...
    assert "not a verified CAD model" in pkg["final_answer"]
...
def test_ocr_opencv_fusion_cards_are_added_to_visual_package() -> None:
    ...
    assert "OCR/OpenCV + technical-geometry" in pkg["final_answer"]
    assert "OCR text candidates" in pkg["final_answer"]
    assert "Diagram draft generated from OCR/OpenCV/geometry-fused visual package" in pkg["final_answer"]
...
def test_technical_drawing_candidate_builds_geometry_cards() -> None:
    pkg = build_visual_package(
        user_query="Turn this engineering drawing into a technical diagram draft.",
        synthetic_image_id="demo_technical_drawing_001",
        synthetic_visual_type="technical_drawing_candidate",
    )
    assert pkg["response_mode"] == "technical_drawing_draft_guidance"
    assert pkg["technical_geometry_card_count"] == 1
    assert pkg["technical_drawing_candidate_count"] == 1
    assert pkg["technical_drawing_feature_card_count"] == 1
    assert pkg["technical_drawing_feature_count"] >= 1
    assert pkg["diagram_draft_card_count"] == 1
    card = pkg["diagram_draft_cards"][0]
    assert card["diagram_type"] == "technical_drawing_geometry_draft"
    assert card["diagram_format"] == "technical_json_and_mermaid"
    assert card["proof_authority"] is False
    assert "technical_drawing_json" in card
    assert "Uploaded engineering drawing" in card["mermaid"]
    assert "not a verified CAD model" in pkg["final_answer"]


def test_quality_checks_include_technical_drawing_thresholds(tmp_path: Path) -> None:
    report = build_report(output_dir=tmp_path, include_standard_demo_queries=True)
    checks = evaluate_quality(
        report,
        min_sample_queries=5,
        ...
        min_visual_packages=5,
        min_image_quality_cards=5,
        min_ocr_text_cards=5,
        min_opencv_layout_cards=5,
        min_technical_geometry_cards=5,
        min_technical_drawing_candidates=1,
        min_technical_drawing_feature_cards=1,
        min_grounded_visual_packages=5,
        min_visual_observation_cards=5,
        min_llava_observer_cards=5,
        min_guidance_only_visual_cards=5,
        min_self_rag_samples=5,
        min_crag_samples=5,
        min_diagram_draft_cards=1,
        min_guidance_only_diagram_drafts=1,
        max_visual_proof_authority_violations=0,
        max_unsupported_visual_claim_count=0,
        max_post_gate_issue_count=0,
        max_answer_permission_count=0,
