...
def test_vector_smoke_run_wrapper_adds_repo_root_and_imports_existing_main() -> None:
    text = Path("scripts/benchmark/s6_retrieval/run_trace_net_vector_search_smoke_v1.py").read_text(encoding="utf-8")
...
def test_vector_smoke_quality_wrapper_adds_repo_root_and_imports_existing_quality_main() -> None:
    text = Path("scripts/benchmark/s6_retrieval/check_trace_net_vector_search_smoke_v1_quality.py").read_text(encoding="utf-8")
    assert "Path(__file__).resolve().parents[2]" in text
    assert "sys.path.insert(0, str(REPO_ROOT))" in text
