...
def test_build_script_injects_repo_root_before_tiff_import():
    path = Path("scripts/benchmark/s6_retrieval/build_trace_net_table_exact_search_smoke_v1.py")
...
def test_quality_script_injects_repo_root_before_tiff_import():
    path = Path("scripts/benchmark/s6_retrieval/check_trace_net_table_exact_search_smoke_v1_quality.py")
    text = path.read_text(encoding="utf-8")
    assert "REPO_ROOT = Path(__file__).resolve().parents[2]" in text
    assert "sys.path.insert(0, str(REPO_ROOT))" in text
