...
from tiff.trace_net_table_ocr_bbox_enrichment_v1 import (
    ...
    parse_hocr,
    parse_tsv,
    tighten_ocr_bbox_to_content_band,
    union_bboxes,
)
...
def make_image(path: Path, size=(1000, 1200)):
    from PIL import Image

    Image.new("L", size, 255).save(path)


def thresholds(min_crop_candidate_cards=1):
    return {
        "min_source_cards": 1,
        "min_enrichment_cards": 1,
        "min_crop_candidate_cards": min_crop_candidate_cards,
...
def test_parse_hocr_extracts_bbox_text(tmp_path):
    ...
    p.write_text(
        '<span class="ocrx_word" title="bbox 1 2 30 10; x_wconf 95">ABC</span>',
        encoding="utf-8",
    )
    rows = parse_hocr(p)
    assert len(rows) == 1
...
def test_build_report_matches_part_number_ocr_bbox(tmp_path):
        ...
        "table_geometry_cards": [
            {
                ...
                "domain_validation": {"part_numbers_sample": ["120-46137-001"]},
            }
        ],
    }
    normalizer = {
        "quality_status": "PASS",
        "records": [{"page_id": "t_p_120_1176_p000003", "table_id": "t1", "text": "120-46137-001"}],
    }
    image_resolver = {
        "quality_status": "PASS",
    ...
    bbox_resolver = {
        "quality_status": "PASS",
        "table_bbox_cards": [{"page_id": "t_p_120_1176_p000003", "table_id": "t1", "review_required": False}],
    }
    ...
    assert card["table_extraction_bbox_preferred"] is False


def test_build_report_prefers_table_extraction_bbox_over_ocr_union(tmp_path):
    ocr_root = tmp_path / "ocr"
    ocr_root.mkdir()
    (ocr_root / "zip_page_000007_00000007.tsv").write_text(
        "level\tleft\ttop\twidth\theight\tconf\ttext\n"
        "5\t10\t10\t900\t40\t90\tHEADER\n"
        "5\t100\t200\t120\t40\t90\t120-46137-001\n"
        "5\t200\t1100\t750\t40\t90\tFOOTER\n",
        encoding="utf-8",
    )
    img = tmp_path / "zip_page_000007_00000007.tif"
    make_image(img)
    extraction_bbox = {"x0": 90, "y0": 180, "x1": 720, "y1": 520}
    tlg = {
        "quality_status": "PASS",
        "table_geometry_cards": [
            {
                "geometry_card_id": "g1",
                "page_id": "t_p_120_1176_p000007",
                "table_id": "t1",
                "table_type": "parts_list_table",
                "resolved_image_path": str(img),
                "image_width": 1000,
                ...
                "table_extraction_bbox_source": "paddle_style_table_bbox_resolver",
                "table_extraction_bbox_confidence": 0.93,
                "domain_validation": {"part_numbers_sample": ["120-46137-001"]},
            }
        ],
    }
    normalizer = {
        "quality_status": "PASS",
        "records": [
            {"page_id": "t_p_120_1176_p000007", "table_id": "t1", "text": "120-46137-001"},
            {"page_id": "t_p_120_1176_p000007", "table_id": "t1", "text": "HEADER"},
            {"page_id": "t_p_120_1176_p000007", "table_id": "t1", "text": "FOOTER"},
        ],
    }
    image_resolver = {
        "quality_status": "PASS",
    ...
    bbox_resolver = {
        "quality_status": "PASS",
        "table_bbox_cards": [{"page_id": "t_p_120_1176_p000007", "table_id": "t1", "review_required": False}],
    }
    write_json(tmp_path / "tlg.json", tlg)
    write_json(tmp_path / "norm.json", normalizer)
    write_json(tmp_path / "img.json", image_resolver)
    ...
    assert report["quality_status"] == "PASS"
    card = report["table_ocr_bbox_enrichment_cards"][0]
    assert card["bbox_source"] == "table_extraction_bbox_preferred"
    assert card["inferred_table_region_bbox"]["x0"] == 90
    assert card["inferred_table_region_bbox"]["y1"] == 520
    assert card["ocr_inferred_table_region_bbox"] is not None
    assert card["table_extraction_bbox_available"] is True
    assert card["table_extraction_bbox_valid"] is True
    assert card["table_extraction_bbox_preferred"] is True
    assert card["crop_candidate_ready"] is True
    assert "use_table_extraction_bbox_for_downstream_table_ocr_crop" in card["recommended_actions"]
    assert report["summary"]["table_extraction_bbox_available_card_count"] == 1
    assert report["summary"]["table_extraction_bbox_valid_card_count"] == 1
    assert report["summary"]["table_extraction_bbox_preferred_card_count"] == 1
    assert report["summary"]["table_extraction_bbox_consumed_card_count"] == 1
...
def test_build_report_passes_with_missing_ocr_sidecar_when_threshold_zero(tmp_path):
    tlg = {
        "quality_status": "PASS",
        "table_geometry_cards": [{"geometry_card_id": "g1", "page_id": "p000001", "table_id": "t1"}],
    }
...
def test_content_band_tightening_reduces_broad_ocr_union(tmp_path):
    ...
    for i in range(30):
        ...
        records.append(
            {
                "text": f"row-{i}",
                "bbox": {"x0": 200, "y0": y, "x1": 900, "y1": y + 12, "width": 700, "height": 12},
            }
        )
    records.extend(
        [
            ...
            {
                "text": "footer",
                "bbox": {"x0": 100, "y0": 1180, "x1": 980, "y1": 1200, "width": 880, "height": 20},
            },
        ]
    )
...
def test_build_report_records_content_band_fields_for_broad_ocr_bbox(tmp_path):
    ...
    bbox_resolver = {
        "quality_status": "PASS",
        "table_bbox_cards": [{"page_id": "t_p_120_1176_p000005", "table_id": "t1", "review_required": False}],
    }
    write_json(tmp_path / "tlg.json", tlg)
    write_json(tmp_path / "norm.json", normalizer)
    write_json(tmp_path / "img.json", image_resolver)
