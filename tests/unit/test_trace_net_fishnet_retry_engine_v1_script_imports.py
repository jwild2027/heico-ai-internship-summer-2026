...
def test_quality_script_imports() -> None:
    path = Path("scripts/maintenance/benchmark/check_trace_net_fishnet_retry_engine_v1_quality.py")
    if not path.exists():
        path = Path(__file__).resolve().parents[2] / "scripts/maintenance/benchmark/check_trace_net_fishnet_retry_engine_v1_quality.py"
