...
def test_v33_always_calls_llm_in_simulate_mode(tmp_path: Path):
...
def test_v33_v2_summary_count_package_has_metadata(tmp_path: Path):
    table, page_context, leiden, router, hardener = _fixtures(tmp_path)
    writer = TraceNetGemmaAnswerWriterV33.from_paths(
        table_exact_search_adapter=table,
        page_context_v2=page_context,
...
def test_v33_relationship_question_is_guidance_only(tmp_path: Path):
    table, page_context, leiden, router, hardener = _fixtures(tmp_path)
    writer = TraceNetGemmaAnswerWriterV33.from_paths(
        table_exact_search_adapter=table,
        page_context_v2=page_context,
...
def test_v33_final_gate_repairs_unsafe_draft(tmp_path: Path):
    table, page_context, leiden, router, hardener = _fixtures(tmp_path)
    writer = TraceNetGemmaAnswerWriterV33.from_paths(
        table_exact_search_adapter=table,
        page_context_v2=page_context,
...
def test_v33_build_report_quality(tmp_path: Path):
    table, page_context, leiden, router, hardener = _fixtures(tmp_path)
    report = build_report(
        table_exact_search_adapter=table,
        page_context_v2=page_context,
...
def test_v33_compact_prompt_telemetry_and_budget(tmp_path: Path):
    table, page_context, leiden, router, hardener = _fixtures(tmp_path)
    writer = TraceNetGemmaAnswerWriterV33.from_paths(
        table_exact_search_adapter=table,
        page_context_v2=page_context,
...
def test_v33_missing_normal_intents_are_packaged_and_answered(tmp_path: Path):
    table, page_context, leiden, router, hardener = _fixtures(tmp_path)
    writer = TraceNetGemmaAnswerWriterV33.from_paths(
        table_exact_search_adapter=table,
        page_context_v2=page_context,
...
def test_v33_build_report_counts_normal_intents(tmp_path: Path):
    table, page_context, leiden, router, hardener = _fixtures(tmp_path)
    report = build_report(
        table_exact_search_adapter=table,
        page_context_v2=page_context,
...
def test_v33_self_rag_and_crag_telemetry_present(tmp_path: Path):
    table, page_context, leiden, router, hardener = _fixtures(tmp_path)
    writer = TraceNetGemmaAnswerWriterV33.from_paths(
        table_exact_search_adapter=table,
        page_context_v2=page_context,
    ...
    resp = writer.answer_query("Find part number 120-36833-503", llm_mode="simulate")
    tn = resp["trace_net"]
    assert tn["self_rag_status"] == "SELF_RAG_SOURCE_TRUTH_READY"
    assert tn["self_rag_package_quality"] == "strong"
    assert tn["self_rag_answerable_from_package"] is True
    assert tn["self_rag_direct_source_truth_available"] is True
    assert tn["crag_status"] == "CRAG_NO_RETRY_SOURCE_TRUTH_OR_METADATA_READY"
    assert tn["crag_retry_required"] is False


def test_v33_page_profile_combines_source_truth_and_v2_guidance(tmp_path: Path):
    table, page_context, leiden, router, hardener = _fixtures(tmp_path)
    writer = TraceNetGemmaAnswerWriterV33.from_paths(
        table_exact_search_adapter=table,
        page_context_v2=page_context,
    ...
    resp = writer.answer_query("What do we know about page t_p_120_1176_p000003?", llm_mode="simulate")
    text = resp["choices"][0]["message"]["content"]
    tn = resp["trace_net"]
    assert "source-truth record" in text
    assert "120-36833-503" in text
    assert "Parts listing page" in text
    assert "guidance" in text.lower()
    assert tn["self_rag_status"] in {"SELF_RAG_SOURCE_TRUTH_READY", "SELF_RAG_SOURCE_TRUTH_READY_WITH_CAP_DISCLOSURE"}
    assert tn["crag_retry_required"] is False


def test_v33_build_report_counts_self_rag_and_crag(tmp_path: Path):
    table, page_context, leiden, router, hardener = _fixtures(tmp_path)
    report = build_report(
        table_exact_search_adapter=table,
        page_context_v2=page_context,
        ...
        min_compact_prompt_samples=8,
        min_normal_intent_samples=6,
        min_self_rag_samples=8,
        min_crag_samples=8,
        max_crag_retry_required_count=1,
        max_post_gate_issue_count=0,
        max_answer_permission_count=0,
        max_source_truth_mutation_allowed=0,
        require_no_answer_permission=True,
    ...
    assert report["quality_status"] == "PASS"
    assert report["self_rag_sample_count"] == report["sample_query_count"]
    assert report["crag_sample_count"] == report["sample_query_count"]
    assert "strong" in report["self_rag_quality_counts"] or "partial" in report["self_rag_quality_counts"]
