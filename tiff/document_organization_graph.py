...
def _read_context_file(context_file: str | Path | None) -> tuple[list[dict[str, Any]], list[str]]:
    """Load optional PageContext records generated by scripts/generate_page_contexts.py."""
    warnings: list[str] = []
    if not context_file:
        return [], warnings
    ...
    try:
        ...
        warnings.append(f"could not read page context file {path}: {exc}")
        return [], warnings
    contexts = data.get("contexts") if isinstance(data, dict) else data
    if not isinstance(contexts, list):
        warnings.append(f"page context file did not contain a context list: {path}")
        return [], warnings
    return [x for x in contexts if isinstance(x, dict)], warnings


def _context_page_id(context: dict[str, Any]) -> str:
    return _norm_text(_first_nonempty(context, ("page_id", "page", "id"), ""))


def _context_id(context: dict[str, Any]) -> str:
    raw = _norm_text(_first_nonempty(context, ("context_id", "id"), ""))
    if raw:
        return raw if raw.startswith("page_context:") else _node_id("page_context", raw)
    return _node_id("page_context", _context_page_id(context))
...
def build_graph_from_export(export_dir: str | Path, strict: bool = False, context_file: str | Path | None = DEFAULT_CONTEXT_FILE) -> GraphBuildResult:
    export_path = Path(export_dir)
    data, warnings = _read_export_files(export_path, strict=strict)
    context_records, context_warnings = _read_context_file(context_file)
    ...
    # Add optional AI-generated PageContext nodes. These summarize pages but do not
    # replace source OCR/TIFF truth. Context nodes are derived and auditable.
    for context in context_records:
        pid = _context_page_id(context)
        if not pid:
            warnings.append("skipped page context with no page_id")
            continue
        page_node = _node_id("page", pid)
        if page_node not in nodes:
            _add_node(nodes, page_node, "page", pid, {"page_id": pid, "stub": True})
        context_node = _context_id(context)
        summary_text = _norm_text(_first_nonempty(context, ("short_summary", "summary", "text"), ""))
        topics = _as_text_list(context.get("topics"))
        important_parts = _as_text_list(_first_nonempty(context, ("important_parts", "parts"), []))
        _add_node(
            nodes,
            context_node,
            "page_context",
            summary_text[:120] if summary_text else f"context for {pid}",
            {
                "page_id": pid,
                "short_summary": summary_text,
                "page_role": _first_nonempty(context, ("page_role", "role"), None),
                "topics": topics,
                "important_parts": important_parts,
                "confidence": _first_nonempty(context, ("confidence",), None),
                "model": _first_nonempty(context, ("model", "generated_by"), None),
                "prompt_version": _first_nonempty(context, ("prompt_version",), None),
                "generated_at": _first_nonempty(context, ("generated_at",), None),
                "source_ocr_hash_hint": _first_nonempty(context, ("source_ocr_hash_hint",), None),
                "error": _first_nonempty(context, ("error",), None),
            },
        )
        _add_edge(edges, "HAS_CONTEXT", page_node, context_node)
        _add_edge(edges, "SUMMARIZES", context_node, page_node)

        ocr_path = _norm_text(_first_nonempty(context, ("ocr_path", "source_ocr_path"), ""))
        if ocr_path:
            ocr_node = _node_id("source_file", f"ocr:{ocr_path}")
            if ocr_node in nodes:
                _add_edge(edges, "GENERATED_FROM", context_node, ocr_node)

        for part_number in important_parts:
            part_node = _node_id("part", part_number)
            if part_node in nodes:
                _add_edge(edges, "HIGHLIGHTS_PART", context_node, part_node)

        for topic in topics:
            topic_node = _node_id("topic", topic)
            _add_node(nodes, topic_node, "topic", topic, {"topic": topic})
            _add_edge(edges, "TAGGED_AS", context_node, topic_node)
    ...
    summary = {
        ...
        "input_counts": {
            ...
            "page_contexts": len(context_records),
        },
        "graph_counts": {
...
def export_graph(export_dir: str | Path, output_dir: str | Path, strict: bool = False, context_file: str | Path | None = DEFAULT_CONTEXT_FILE) -> GraphBuildResult:
    result = build_graph_from_export(export_dir, strict=strict, context_file=context_file)
