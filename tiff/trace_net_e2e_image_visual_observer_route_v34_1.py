...
VERSION = "v34.1"
MODULE = "trace_net_e2e_image_visual_observer_route_v34_1"
MODEL_ID = "trace-net-e2e-image-diagram-draft-llava-v34-1"
...
def _build_diagram_draft_card(query: str, package: Mapping[str, Any]) -> Dict[str, Any] | None:
    """Build a text/JSON diagram draft from the visual package.

    This deliberately avoids image generation. It produces a Mermaid draft and a
    structured diagram JSON object that the WebUI/user can inspect or render.
    The draft is guidance only because visual observations are not source truth.
    """
    ...
    def add_node(node_id: str, label: str, kind: str, confidence: str = "medium") -> None:
        ...
        nodes.append({
            ...
            "source": "llava_visual_observer_guidance",
            "proof_authority": False,
            "requires_source_truth_confirmation": True,
        })

    # Conservative domain-ish sketching from visible-observation terms. This is
    # intentionally generic and does not assert exact part identity.
    if "engine" in low or "powerplant" in low or "nacelle" in low:
        add_node("engine_or_powerplant_area", "Engine / powerplant area", "visual_region", "medium")
    if "intake" in low or "fan" in low or "blade" in low or "turbine" in low:
        add_node("intake_fan_area", "Intake / fan area", "visual_subregion", "medium")
    if "wing" in low or "pylon" in low or "attachment" in low:
        add_node("wing_pylon_area", "Wing / pylon area", "visual_region", "low")
    if "propeller" in low or "aircraft" in low or "airplane" in low or "plane" in low:
        add_node("aircraft_or_propeller_area", "Aircraft / propeller area", "visual_region", "medium")
    if "text" in low or "label" in low or "aircraft parts" in low or "powerplant" in low:
        add_node("visible_text_region", "Visible text region", "text_region", "medium")
    if "arrow" in low or "callout" in low or "leader" in low:
        add_node("possible_callouts", "Possible callouts / leader lines", "annotation_region", "low")

    if not nodes:
        add_node("uploaded_image", "Uploaded image", "image", "low")
        add_node("main_visible_subject", "Main visible subject", "visual_region", "low")
    ...
    def add_edge(src: str, dst: str, label: str) -> None:
        ...
        edges.append({
            ...
            "to": dst,
            "label": label,
            "confidence": "low" if "possible" in label.lower() else "medium",
            "source": "diagram_draft_guidance",
            "proof_authority": False,
        })

    if any(n["id"] == "engine_or_powerplant_area" for n in nodes):
        for dst in ("intake_fan_area", "wing_pylon_area", "visible_text_region"):
            add_edge("engine_or_powerplant_area", dst, "visually associated")
    if any(n["id"] == "aircraft_or_propeller_area" for n in nodes):
        add_edge("aircraft_or_propeller_area", "visible_text_region", "shares graphic")
    if any(n["id"] == "possible_callouts" for n in nodes):
        for dst in [n["id"] for n in nodes if n["id"] != "possible_callouts"][:3]:
            add_edge("possible_callouts", dst, "may point to")
    if not edges and len(nodes) > 1:
        add_edge(nodes[0]["id"], nodes[1]["id"], "visually related")

    mermaid_lines = ["flowchart LR"]
    for n in nodes:
        mermaid_lines.append(f'  {n["id"]}["{n["label"]}"]')
    for e in edges:
        mermaid_lines.append(f'  {e["from"]} -- "{e["label"]}" --> {e["to"]}')

    return {
        "diagram_draft_id": _stable_id("diagram_draft_v34_1", query + json.dumps(nodes, sort_keys=True)),
        "diagram_format": "mermaid_and_json",
        "authority": "guidance_only",
        "proof_authority": False,
        "requires_source_truth_confirmation": True,
        "human_review_recommended": True,
        "diagram_type": "visual_structure_draft",
        "nodes": nodes,
        "edges": edges,
        "mermaid": "\n".join(mermaid_lines),
        "warnings": [
            "This diagram draft is generated from visual observations only.",
...
def _visual_safe_answer(package: Mapping[str, Any]) -> str:
    ...
    if diagram_cards and _wants_diagram_draft(query):
        ...
        return (
            base
            + "\n\nDiagram draft generated from the visual package (guidance only):"
            + "\n\n```mermaid\n"
            + mermaid
            + "\n```\n\n"
            + f"Draft nodes: {node_list}. "
            + "This is a diagram draft, not a verified technical drawing or source-truth replacement."
        )
    return base
...
def build_report(
    *,
    ...
    report_path = outdir / "trace_net_e2e_image_visual_observer_route_v34_1.json"
    records_jsonl_path = outdir / "trace_net_e2e_image_visual_observer_route_records_v34_1.jsonl"
    inspect_md_path = outdir / "trace_net_e2e_image_visual_observer_route_v34_1.md"
