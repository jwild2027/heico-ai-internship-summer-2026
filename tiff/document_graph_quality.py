...
JSON artifacts produced by ``scripts/export_document_organization_graph.py`` and
    ...
    *,
    ...
    _add_check(
        ...
        (
            ...
            if user_query_ok
            else "User-query regression results are missing or not all passing. Run scripts/run_user_query_tests.py --config local_config.yaml --write-json."
        ),
    )
    ...
    _add_check(
        ...
        (
            ...
            if realistic_ok
            else "Realistic query trace results are missing or not all passing. Run scripts/run_realistic_query_trace_tests.py --config local_config.yaml --include-slow --write-json."
        ),
    )
    _add_check(
        checks,
