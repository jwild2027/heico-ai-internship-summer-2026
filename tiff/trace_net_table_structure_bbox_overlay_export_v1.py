...
Read-only PNG overlay exporter for the structure-first table bbox localizer.

This module consumes ``trace_net_table_structure_bbox_localizer_v1`` JSON output and
renders inspectable PNG overlays plus a contact sheet that compares the
upstream input bbox, rejected/accepted visual candidate, and final
structure-selected bbox. It is intentionally a QA
...
SCHEMA_VERSION = "trace_net_table_structure_bbox_overlay_export_v1"
QUALITY_SCHEMA_VERSION = "trace_net_table_structure_bbox_overlay_export_v1_quality"
STATUS_BUILT = "TABLE_STRUCTURE_BBOX_OVERLAY_EXPORT_BUILT"
STATUS_NOT_READY = "TABLE_STRUCTURE_BBOX_OVERLAY_EXPORT_NOT_READY"
...
def source_records(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    for key in ("table_structure_bbox_localizer_records", "structure_localization_records", "records"):
...
def bbox_coverage(box: Mapping[str, Any] | None, width: float | None, height: float | None) -> float | None:
    if not box or not width or not height or width <= 0 or height <= 0:
        return None
    x0, y0, x1, y1 = (as_float(box.get(k)) for k in ("x0", "y0", "x1", "y1"))
    if x0 is None or y0 is None or x1 is None or y1 is None:
        return None
    area = max(0.0, x1 - x0) * max(0.0, y1 - y0)
    return round(area / (float(width) * float(height)), 6)
...
def render_overlay(record: Mapping[str, Any], image_path: Path, output_path: Path, max_dimension: int = 1800) -> dict[str, Any]:
    ...
    visual_box = bbox_from_mapping(record.get("visual_candidate_bbox"))
    selected_box = bbox_from_mapping(record.get("structure_selected_table_bbox"))
    if not selected_box:
        return {"overlay_written": False, "overlay_error": "missing_structure_selected_table_bbox"}
    ...
    # Amber = upstream/input bbox. Blue = accepted visual candidate; red = rejected visual candidate.
    # Green = final structure-selected bbox. If fallback is selected, green intentionally overlaps amber.
    if input_box:
        draw_rect(draw, scaled_bbox(input_box, scale), (255, 191, 0), width=5)
        draw_label(draw, "input bbox", (10, 10), (120, 82, 0))
    if visual_box:
        candidate_color = (40, 110, 230) if record.get("structure_visual_candidate_accepted") else (210, 55, 55)
        draw_rect(draw, scaled_bbox(visual_box, scale), candidate_color, width=4)
        draw_label(draw, "visual candidate", (10, 38), candidate_color)
    draw_rect(draw, scaled_bbox(selected_box, scale), (0, 190, 70), width=7)
    draw_label(draw, "structure-selected bbox", (10, 66), (0, 140, 60))

    selected_cov = bbox_coverage(selected_box, original_w, original_h)
    caption = " | ".join(
        item
        for item in [
            str(record.get("page_id") or ""),
            str(record.get("structure_selected_bbox_source") or ""),
            f"visual_accepted={record.get('structure_visual_candidate_accepted')}",
            f"selected_cov={selected_cov}",
        ]
        if item
    )
    draw_label(draw, caption[:170], (10, max(94, image.height - 28)), (0, 0, 0))
    ...
    return {
        ...
        "structure_selected_bbox_coverage_ratio": selected_cov,
    }
...
def make_contact_sheet(overlay_records: Sequence[Mapping[str, Any]], output_path: Path, columns: int = 2, thumb_width: int = 720) -> dict[str, Any]:
    ...
    for record in usable:
        ...
        label = f"{record.get('page_id')} | pass={record.get('structure_selected_bbox_ready')} | {record.get('structure_selected_bbox_source')}"
...
def overlay_filename(record: Mapping[str, Any], index: int) -> str:
    ...
    return f"{index:03d}_{page}_{table}_structure_bbox_overlay.png"
...
def build_overlay_record(record: Mapping[str, Any], *, index: int, image_root: Path | None, image_index: Mapping[str, Path], overlays_dir: Path, max_dimension: int) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "overlay_export_id": stable_id("tblstructoverlay", page_id, table_id, index),
        "schema_version": SCHEMA_VERSION,
        "page_id": page_id,
        "table_id": table_id,
        "source_structure_bbox_localizer_id": record.get("table_structure_bbox_localizer_id"),
        ...
        "input_bbox": record.get("input_bbox"),
        "visual_candidate_bbox": record.get("visual_candidate_bbox"),
        "visual_candidate_bbox_source": record.get("visual_candidate_bbox_source"),
        "structure_selected_table_bbox": record.get("structure_selected_table_bbox"),
        "structure_selected_bbox_source": record.get("structure_selected_bbox_source"),
        "structure_selected_bbox_key": record.get("structure_selected_bbox_key"),
        "structure_selected_bbox_ready": record.get("structure_selected_bbox_ready"),
        "structure_visual_candidate_accepted": record.get("structure_visual_candidate_accepted"),
        "structure_visual_candidate_rejected": record.get("structure_visual_candidate_rejected"),
        ...
        "unsafe_table_structure_bbox_overlay_record": False,
    }
...
def summarize(records: Sequence[Mapping[str, Any]], source_quality: str | None, contact_sheet: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "source_table_structure_bbox_localizer_quality_status": source_quality,
        ...
        "structure_selected_ready_overlay_count": sum(1 for r in records if r.get("structure_selected_bbox_ready")),
        "conservative_fallback_overlay_count": sum(1 for r in records if r.get("structure_selected_bbox_source") == "conservative_input_bbox_fallback"),
        "structure_visual_accepted_overlay_count": sum(1 for r in records if r.get("structure_visual_candidate_accepted")),
        "structure_visual_rejected_overlay_count": sum(1 for r in records if r.get("structure_visual_candidate_rejected")),
        ...
        "unsafe_table_structure_bbox_overlay_record_count": sum(1 for r in records if r.get("unsafe_table_structure_bbox_overlay_record")),
...
def quality_errors(summary: Mapping[str, Any], args: argparse.Namespace) -> list[str]:
    ...
    if args.require_table_structure_bbox_localizer_quality_pass and summary.get("source_table_structure_bbox_localizer_quality_status") != "PASS":
        errors.append("source_table_structure_bbox_localizer_quality_status_not_pass")
    ...
    if get_count("unsafe_table_structure_bbox_overlay_record_count") > args.max_unsafe_records:
        errors.append("unsafe_table_structure_bbox_overlay_record_count_above_max")
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    source_path = Path(args.table_structure_bbox_localizer)
    ...
    contact_sheet_path = output_dir / "trace_net_table_structure_bbox_localizer_overlay_contact_sheet_v1.png"
    ...
    payload: dict[str, Any] = {
        ...
        "table_structure_bbox_overlay_export_records": overlay_records,
        "overlay_records": overlay_records,
    }

    report_path = output_dir / "trace_net_table_structure_bbox_overlay_export_v1.json"
    records_path = output_dir / "trace_net_table_structure_bbox_overlay_export_v1_records.jsonl"
    summary_path = output_dir / "trace_net_table_structure_bbox_overlay_export_v1_summary.json"
    quality_path = output_dir / "trace_net_table_structure_bbox_overlay_export_v1_quality.json"
    manifest_path = output_dir / "trace_net_table_structure_bbox_overlay_export_v1_manifest.json"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build TRACE-Net table structure bbox overlay export v1")
    parser.add_argument("--table-structure-bbox-localizer", required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("TRACE-Net Table Structure BBox Overlay Export v1")
    ...
    print(f" report_path: {Path(args.output_dir) / 'trace_net_table_structure_bbox_overlay_export_v1.json'}")
