...
def _top_counts(audit: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not counts:
        counts = _as_dict(_as_dict(audit.get("summary")).get("counts"))
    return counts
...
def _top_signals(audit: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not signals:
        signals = _as_dict(_as_dict(audit.get("summary")).get("image_recognition_signals"))
    return signals
...
def _top_classes(audit: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not classes:
        classes = _as_dict(_as_dict(audit.get("summary")).get("classification_counts"))
    return classes
...
def _top_roles(audit: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not roles:
        roles = _as_dict(_as_dict(audit.get("summary")).get("page_roles"))
    return roles
...
def _rows(audit: Mapping[str, Any]) -> list[Any]:
    for key in ("rows", "records", "pages", "sample_rows"):
        ...
        if isinstance(value, list):
            return value
    # Some tools write a nested detail block.  Only use it if it is clearly a list of page records.
...
def _overlay_paths_from_audit(audit: Mapping[str, Any]) -> tuple[Any, Any]:
    ...
    nodes = _first(overlay, "nodes", "nodes_path", "graph_nodes", "graph_nodes_path", default=None)
    edges = _first(overlay, "edges", "edges_path", "graph_edges", "graph_edges_path", default=None)
    if not nodes:
        files = _as_dict(audit.get("graph_overlay_files"))
        nodes = _first(files, "nodes", "nodes_path", default=None)
        edges = _first(files, "edges", "edges_path", default=edges)
    return nodes, edges
...
def summarize_page_image_recognition_audit(
    ...
    *,
    ...
    status = _norm_status(audit.get("status") or _as_dict(audit.get("summary")).get("status"))
    pages_checked = _to_int(_first(counts, "pages_checked", "pages", "page_count", "total_pages"))
    readable = _to_int(_first(counts, "readable_images", "readable_image_files", "images_readable"))
    ...
    likely_figure = _to_int(_first(signals, "likely_figure_diagram_pages", "likely_figure_or_diagram_pages", "likely_figure_pages", "figure_pages"))
    likely_table = _to_int(_first(signals, "likely_table_grid_pages", "likely_table_or_grid_pages", "likely_table_pages", "table_pages"))
    likely_image_heavy = _to_int(_first(signals, "likely_image_heavy_pages", "image_heavy_pages"))
    likely_text = _to_int(_first(signals, "likely_text_parts_list_pages", "likely_text_or_parts_list_pages", "likely_text_pages"))
...
def build_page_image_recognition_quality_report(
    ...
    *,
    ...
    if not path.exists():
        ...
        checks = [_check("page_image_audit_present", False, f"Page image-recognition audit JSON is missing at {path}.")]
    ...
    if not isinstance(audit, dict):
        ...
        checks = [_check("page_image_audit_status", False, "Page image-recognition audit JSON is not an object.")]
        return QualityReport(status="FAIL", summary=summary, checks=checks)
    ...
    checks = [
        ...
        _check("page_image_classification_counts", classified == pages, f"Classified pages={classified}; pages checked={pages}."),
        _check("page_image_role_counts", role_pages == pages and unknown_roles == 0, f"Role pages={role_pages}; unknown roles={unknown_roles}; pages checked={pages}."),
        ...
        _check("page_image_table_signals", likely_table > 0 and table_class > 0, f"Likely table/grid pages={likely_table}; table/grid classifications={table_class}."),
        ...
        _check("page_image_graph_overlay_nodes", overlay_nodes_present and overlay_nodes > 0, f"Graph overlay nodes present={overlay_nodes_present} count={overlay_nodes}."),
        _check("page_image_graph_overlay_edges", overlay_edges_present and overlay_edges > 0, f"Graph overlay edges present={overlay_edges_present} count={overlay_edges}."),
    ]
