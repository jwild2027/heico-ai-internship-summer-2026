...
This is v3 of the local TIFF-content Gemma runner. It does not use the old
...
sends question + evidence to Ollama/Gemma, and writes Question/Answer output. V3 also creates deterministic evidence summaries/fallbacks so useful route evidence is not thrown away when the model returns a blank answer.
...
def make_prompt(question_id: str, question: str, route: str, evidence: list[EvidenceRecord], deterministic_draft: str | None = None) -> str:
    ...
    return f"""TRACE-NET TIFF-CONTENT QA WORK ORDER — ROUTED EVIDENCE V3
...
def evidence_parts(evidence: list[EvidenceRecord], limit: int = 12) -> list[str]:
    cleaned: list[str] = []
    for e in evidence:
        for part in e.part_numbers:
            # Drop obvious page counters and bare document numbers from deterministic summaries.
            pn = part.strip()
            if re.fullmatch(r"\d{5,8}", pn):
                continue
            cleaned.append(pn)
...
def deterministic_answer(question: str, route: str, evidence: list[EvidenceRecord]) -> str | None:
    ...
    This is used as a draft/fallback when Gemma returns blank or when the route
    has a simple structured answer (ATA/title/revision/pages/parts). It never
    invents facts: it only summarizes already selected evidence metadata/text.
    """
    if not evidence:
        return None
    qlow = question.lower()
    ...
    if route in {"figure_visual", "figure_nomenclature"}:
        ...
        if fig_no:
            ...
            target_noms = []
            for e in evidence:
                blob = f"{e.source_path} {e.text}".lower()
                if re.search(rf"\b(?:figure|fig\.?)[ _-]*{re.escape(fig_no.lower())}\b", blob) or f"figure {fig_no.lower()}" in blob:
                    target_pages.extend(e.page_ids)
                    target_parts.extend(e.part_numbers)
                    target_noms.extend(extract_nomenclature_candidates([e]))
            target_pages = unique_sorted(target_pages, limit=8)
            ...
            if "which page" in qlow or "contains" in qlow:
                if target_pages:
                    return (
                        f"Direct answer: Figure {fig_no} is linked to {', '.join(page_label(p) for p in target_pages)}.\n"
                        "Source-trace status: Source-traceable.\n"
                        f"Evidence used: {ev_refs(6)}.\n"
            ...
            if target_noms or target_parts:
                ...
                return (
                    f"Direct answer: Figure {fig_no} is associated with {' and '.join(detail)}.\n"
                    "Source-trace status: Source-traceable.\n"
                    f"Evidence used: {ev_refs(6)}.\n"
    ...
    if route == "table" and pages:
        return (
            "Direct answer: Selected table evidence appears on " + ", ".join(page_label(p) for p in pages[:10]) + ".\n"
            "Source-trace status: Source-traceable for the selected snippets.\n"
            f"Evidence used: {ev_refs(8)}.\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = {
        "status": "TRACE_NET_TIFF_CONTENT_GEMMA_EVIDENCE_PACK_ROUTER_V3_DONE",
        "quality_status": "PASS" if len(rows) == len(questions) and unresolved_fallback_count == 0 else "WARN",
