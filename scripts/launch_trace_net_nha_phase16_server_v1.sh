...
"$PYTHON_BIN" -B scripts/build_trace_net_nha_engram_v1.py \
  --base-engram-core "$BASE_ENGRAM" \
  --base-skill-library "$BASE_SKILLS" \
  --output-dir "$ENGRAM_DIR" \
  --strict >/dev/null

"$PYTHON_BIN" -B scripts/check_trace_net_nha_engram_v1.py \
  --output-dir "$ENGRAM_DIR" \
  --strict >/dev/null
...
nohup "$PYTHON_BIN" -B scripts/serve_trace_net_nha_phase16_gemma_proxy_v1.py \
  --host "$HOST" \
  --port "$PORT" \
  --mode "$MODE" \
