...
MODULE_NAME = "trace_net_confirmed_image_llava_observations_v1_2"
STATUS_BUILT = "TRACE_NET_CONFIRMED_IMAGE_LLAVA_OBSERVATIONS_V1_2_BUILT"
QUALITY_PASS = "PASS"
QUALITY_FAIL = "FAIL"
...
GENERIC_CALLOUT_SENTENCE_PATTERNS = {
    "callouts on the diagram include",
    "labels and text associated with",
    "various lines and arrows",
    "possibly indicating",
    "possibly some indications",
    "part numbers, measurements",
    "orientation or assembly",
    "the diagram includes",
    "the image contains",
    "there are no legible",
}

GENERIC_SUBJECT_GUESSES = {
    "aircraft structure",
    "aircraft component",
    "aircraft components",
    "aircraft part",
    "aircraft parts",
    "aircraft or vehicle part",
    "aircraft structural components or parts",
...
def is_prompt_leak(value: str) -> bool:
    low = compact_ws(value, limit=600).lower()
    return any(phrase in low for phrase in PROMPT_LEAK_PHRASES)


def is_generic_callout_sentence(value: str) -> bool:
    text = compact_ws(value, limit=600).strip()
    low = text.lower()
    if not text:
        return True
    if is_prompt_leak(text):
        return True
    if len(text) > 80 and not re.search(r"\b[A-Z0-9]{1,6}[-/][A-Z0-9][A-Z0-9./-]{1,24}\b|\b\d{3}-\d{5}-\d{3}\b", text, re.I):
        return True
    if any(pattern in low for pattern in GENERIC_CALLOUT_SENTENCE_PATTERNS):
        return True
    # Long natural-language descriptions are not callout labels.
    if len(text.split()) > 8 and not re.search(r"\b(item|fig(?:ure)?|part|callout)\s*[A-Z0-9-]+\b", text, re.I):
        return True
    return False


def sanitize_subject_guess(value: Any) -> str:
    text = compact_ws(value, limit=400).strip()
    if not text:
        return "unknown"
    low = text.lower()
    if is_prompt_leak(text):
        return "unknown"
    if low in GENERIC_SUBJECT_GUESSES:
        return "unknown"
    # Generic aircraft/mechanical guesses should not be treated as a confident subject.
    if any(x in low for x in ("possibly", "unknown", "not clear", "unclear")):
        return "unknown"
    if low.startswith("aircraft ") and not any(x in low for x in ("seat", "chair", "table", "ashtray")):
        return "unknown"
    return text


def sanitize_figure_text(value: Any) -> str:
    text = compact_ws(value, limit=400).strip()
    low = text.lower()
    if not text or low in {"unknown", "none", "n/a"}:
        return ""
    if any(x in low for x in ("not visible", "not clear", "no clear", "no legible", "unable to")):
        return ""
    if is_prompt_leak(text):
        return ""
    return text
...
def is_boilerplate_or_header_label(value: str) -> bool:
    ...
    if is_prompt_leak(text):
        return True
    if is_generic_callout_sentence(text):
        return True
...
def cleanup_llava_observation(raw_text: str) -> Dict[str, Any]:
    ...
    prompt_leak_values = []
    generic_sentence_values = []
    clean: List[str] = []
    filtered: List[str] = []
    for label in raw_labels:
        if is_prompt_leak(label):
            prompt_leak_values.append(label)
            filtered.append(label)
        elif is_generic_callout_sentence(label):
            generic_sentence_values.append(label)
            filtered.append(label)
        elif is_boilerplate_or_header_label(label):
            filtered.append(label)
        else:
            clean.append(label)

    # Also check non-callout fields for prompt leakage.
    for field_name in ("page_header_or_boilerplate_text", "safety_note", "visual_uncertainty"):
        for value in flatten_label_values(parsed.get(field_name)):
            if is_prompt_leak(value):
                prompt_leak_values.append(value)

    return {
        "parsed": True,
        "visual_page_type": compact_ws(parsed.get("visual_page_type"), limit=300),
        "diagram_subject_guess": sanitize_subject_guess(parsed.get("diagram_subject_guess")),
        "figure_title_or_sheet_text_if_clearly_visible": sanitize_figure_text(
        ...
        "filtered_out_prompt_leak_values": stable_unique(prompt_leak_values, limit=40),
        "filtered_out_generic_callout_sentences": stable_unique(generic_sentence_values, limit=40),
        "page_header_or_boilerplate_text_cleaned": "" if is_prompt_leak(compact_ws(parsed.get("page_header_or_boilerplate_text"), limit=800)) else compact_ws(parsed.get("page_header_or_boilerplate_text"), limit=800),
        ...
        "cleanup_notes": [
            ...
            "Prompt leakage and generic callout sentences are filtered from cleaned callout candidates.",
...
Return ONLY one valid compact JSON object. Do not wrap it in markdown fences. Do not include any prose before or after JSON.
...
- Do NOT copy these instructions, the prompt, or the phrase "TRACE-Net's visual observation specialist" into any output field.
- Do NOT copy the existing hints into your answer unless visibly supported by the image.
- If the diagram subject is not clearly visible, write exactly "unknown".
- Do NOT invent subjects such as engine, aircraft structure, support, mounting point, component, or assembly unless visually obvious from the image.
- visible_callouts_or_labels MUST be a JSON array of short strings.
...
- Do NOT put generic sentences in visible_callouts_or_labels. Bad example: "Callouts include part numbers and measurements."
...
- If visible text is too small or blurry, say that in visual_uncertainty instead of guessing.
...
def summarize(output_jsonl: Path, output_dir: Path, selected_count: int, dry_run: bool) -> Dict[str, Any]:
    ...
    prompt_leak_filtered_count = sum(
        len((r.get("llava_observation_cleaned") or {}).get("filtered_out_prompt_leak_values") or [])
        for r in rows
    )
    generic_callout_sentence_filtered_count = sum(
        len((r.get("llava_observation_cleaned") or {}).get("filtered_out_generic_callout_sentences") or [])
        for r in rows
    )
    ...
    summary = {
        ...
        "filtered_out_prompt_leak_count": prompt_leak_filtered_count,
        "filtered_out_generic_callout_sentence_count": generic_callout_sentence_filtered_count,
        ...
        "outputs": {
            ...
            "report": str(output_dir / "trace_net_confirmed_image_llava_observations_v1_2_report.txt"),
        },
    }
    return summary
...
def build(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    output_jsonl = output_dir / "trace_net_confirmed_image_llava_observations_v1_2.jsonl"
    ...
    report_lines = [
        ...
        f"filtered_out_prompt_leak_count={summary['filtered_out_prompt_leak_count']}",
        f"filtered_out_generic_callout_sentence_count={summary['filtered_out_generic_callout_sentence_count']}",
    ...
    (output_dir / "trace_net_confirmed_image_llava_observations_v1_2_report.txt").write_text(
        "\n".join(report_lines) + "\n",
        encoding="utf-8",
    )
    return summary
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(f"filtered_out_prompt_leak_count={summary['filtered_out_prompt_leak_count']}")
    print(f"filtered_out_generic_callout_sentence_count={summary['filtered_out_generic_callout_sentence_count']}")
