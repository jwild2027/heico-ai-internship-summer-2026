...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.skip_embeddings:
        code = _run_step(
            [sys.executable, "scripts/build/embeddings/build_rag_embeddings.py", "--db-path", str(db_path), "--model", str(embed_model)],
    ...
    if not args.skip_qa:
        code = _run_step([sys.executable, "scripts/maintenance/ingestion/report_part_catalog_qa.py", "--config", args.config], label="part catalog QA")
        if code != 0:
            return code
    ...
    if not args.skip_eval:
        code = _run_step(
            [sys.executable, "scripts/benchmark/ingestion/evaluate_rag_questions.py", "--config", args.config, "--questions", args.questions],
            label="RAG eval",
        )
        if code != 0:
            return code
