...
for required in \
  ...
  scripts/operations/serving/serve_trace_net_blue_green_frontdoor_v1.py; do
...
"$PYTHON" -B src/trace_net/serving/trace_net_blue_green_pointer_v1.py set \
...
exec "$PYTHON" -u -B scripts/operations/serving/serve_trace_net_blue_green_frontdoor_v1.py \\
  --host 127.0.0.1 \\
  --port "$PREVIEW_PORT" \\
  --pointer-path "$POINTER_PATH" \\
