...
This v4 launcher starts the v6 router/proxy so the web UI gets improved
answer-pressure and disambiguation routing from the 50-question smoke.
...
STATUS_READY = "TRACE_NET_ROUTER_STACK_LAUNCHER_V4_READY"
STATUS_STOPPED = "TRACE_NET_ROUTER_STACK_LAUNCHER_V4_STOPPED"
STATUS_FAILED = "TRACE_NET_ROUTER_STACK_LAUNCHER_V4_FAILED"

NORMAL_SCRIPT = Path("scripts/operations/serving/serve_trace_net_e2e_local_endpoint_v1.py")
GUIDED_SCRIPT = Path("scripts/operations/serving/serve_trace_net_guided_candidate_discovery_endpoint_v1.py")
ROUTER_SCRIPT = Path("scripts/operations/router/serve_trace_net_guided_discovery_router_proxy_v6.py")
...
def build_specs(args: argparse.Namespace) -> List[ServiceSpec]:
    ...
    log_dir = Path(args.output_root).expanduser() / "router_stack_launcher_v4" / "logs"
...
def _write_manifest(args: argparse.Namespace, specs: Sequence[ServiceSpec], started: Sequence[ManagedProcess], status: str) -> Path:
    manifest_dir = Path(args.output_root).expanduser() / "router_stack_launcher_v4"
    manifest_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = manifest_dir / "trace_net_router_stack_launcher_v4_manifest.json"
    payload = {
        "status": status,
        ...
        "router_model": "trace-net-router-proxy-v6",
        "safety_contract": {
            "read_only": True,
...
def launch_stack(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print("web_ui_model=trace-net-router-proxy-v6")
