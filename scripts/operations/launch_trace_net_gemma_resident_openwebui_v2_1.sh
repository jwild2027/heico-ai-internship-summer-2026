...
for required in \
  ...
  scripts/operations/s3_graph_store/serve_trace_net_nha_phase16_gemma_proxy_v1.py \
  scripts/operations/serving/serve_trace_net_nha_phase16_gemma_proxy_v1_1.py \
  src/trace_net/serving/trace_net_h30_gemma_residency_watchdog_v2.py \
...
if [[ "$RUN_FOCUSED_TESTS" == "1" ]]; then
  ...
  bash -n \
    scripts/operations/launch_trace_net_gemma_resident_openwebui_v2.sh \
...
exec $(q "$PYTHON") -u -B scripts/operations/writing/serve_trace_net_full_gemma_cognitive_v1.py \
  --host 127.0.0.1 \
  --port 8128 \
...
export TRACE_NET_GEMMA_RENEW_BEFORE_SECONDS=$(q "$RENEW_BEFORE")
export TRACE_NET_GEMMA_PRELOAD_TIMEOUT_SECONDS=$(q "$PRELOAD_TIMEOUT")
exec $(q "$PYTHON") -u -B scripts/operations/serving/serve_trace_net_nha_phase16_gemma_proxy_v1_1.py \
  --host $(q "$BRIDGE_HOST") \
  --port 8131 \
  --mode gated \
