...
"$PYTHON_BIN" -B scripts/maintenance/s5_engram/check_trace_net_nha_engram_v1.py \
  --output-dir "$ENGRAM_DIR" \
  --strict >/dev/null
...
nohup "$PYTHON_BIN" -B scripts/operations/s3_graph_store/serve_trace_net_nha_phase16_gemma_proxy_v1.py \
  --host "$HOST" \
  --port "$PORT" \
  --mode "$MODE" \
