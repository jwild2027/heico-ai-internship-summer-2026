...
def main() -> int:
    ...
    parser.add_argument("--fast-task-types", default="corpus_stats")
    parser.add_argument("--max-evidence-cards", type=int, default=10)
    parser.add_argument("--question", default="how many pages can you look at")
    ...
    log_dir = Path("local_data/organization/trace_net/openwebui_gemma4_engram_bridge_v2/logs")
    log_dir.mkdir(parents=True, exist_ok=True)
    stdout = (log_dir / "bridge_v2_stdout.log").open("a", encoding="utf-8")
    stderr = (log_dir / "bridge_v2_stderr.log").open("a", encoding="utf-8")

    proc = None
    if wait_health(args.port, timeout_seconds=3):
        print("bridge_v2=already_running", flush=True)
    else:
        proc = subprocess.Popen([
            sys.executable, "-B", "scripts/operations/serving/serve_trace_net_openwebui_gemma4_engram_bridge_v2.py",
            "--host", args.host,
            "--port", str(args.port),
            "--ollama-model", args.ollama_model,
            "--llm-policy", args.llm_policy,
            "--fast-task-types", args.fast_task_types,
            "--max-evidence-cards", str(args.max_evidence_cards),
        ], cwd=Path.cwd(), env=env, stdout=stdout, stderr=stderr)
        if not wait_health(args.port, timeout_seconds=80):
            print("status=TRACE_NET_OPENWEBUI_GEMMA4_ENGRAM_V2_LAUNCH_FAILED", flush=True)
            print(f"logs={log_dir}", flush=True)
            return 1

    health = request_json(f"http://127.0.0.1:{args.port}/health", timeout=20)
    ...
    print("status=TRACE_NET_OPENWEBUI_GEMMA4_ENGRAM_V2_READY", flush=True)
    print("quality_status=PASS", flush=True)
    print(f"source_page_count={health.get('source_page_count') or health.get('evidence_card_count')}", flush=True)
