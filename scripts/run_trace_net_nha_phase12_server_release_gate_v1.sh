...
"$PYTHON_BIN" -B scripts/check_trace_net_nha_phase9_release_v1.py \
...
bash scripts/launch_trace_net_nha_phase11_server_v1.sh shadow

"$PYTHON_BIN" -B scripts/run_trace_net_nha_phase11_shadow_http_smoke_v1.py \
...
bash scripts/launch_trace_net_nha_phase11_server_v1.sh gated

LIVE20_DIR="$RUN_ROOT/live20"
rm -rf "$LIVE20_DIR"
"$PYTHON_BIN" -B scripts/run_trace_net_nha_phase10_live20_v1.py \
  --phase4-dir "$RELEASE_DIR" \
  --base-url "$BASE_URL" \
  --api-key "$API_KEY" \
  --model "$MODEL" \
...
"$PYTHON_BIN" -B scripts/check_trace_net_nha_phase10_live20_v1.py \
  --output-dir "$LIVE20_DIR" \
  --expected-count 20 \
  --strict
