...
bash scripts/launch_trace_net_nha_phase16_server_v1.sh shadow
...
bash scripts/launch_trace_net_nha_phase16_server_v1.sh gated

LIVE20_DIR="$RUN_ROOT/gemma20"
rm -rf "$LIVE20_DIR"
"$PYTHON_BIN" -B scripts/run_trace_net_nha_phase16_gemma20_v1.py \
...
"$PYTHON_BIN" -B scripts/check_trace_net_nha_phase16_gemma20_v1.py \
  --output-dir "$LIVE20_DIR" \
  --expected-count 20 \
