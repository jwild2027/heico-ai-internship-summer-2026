...
def main() -> int:
    ...
    if not args.skip_embeddings:
        cmd = [sys.executable, "scripts/build_rag_embeddings.py", "--db-path", str(db_path), "--model", str(embed_model)]
    ...
    if not args.skip_qa:
        cmd = [sys.executable, "scripts/report_part_catalog_qa.py", "--config", args.config]
        print("\nRunning QA report:")
        print("  " + " ".join(cmd))
        proc = subprocess.run(cmd, check=False)
        if proc.returncode != 0:
    ...
    if not args.skip_eval:
        cmd = [sys.executable, "scripts/evaluate_rag_questions.py", "--config", args.config, "--questions", str(questions)]
        print("\nRunning RAG eval:")
        print("  " + " ".join(cmd))
        proc = subprocess.run(cmd, check=False)
        if proc.returncode != 0:
