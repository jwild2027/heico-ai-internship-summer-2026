#!/usr/bin/env python
"""Check TRACE-Net vector search smoke v1 quality.

This wrapper can be executed directly as:
    python scripts/benchmark/s6_retrieval/check_trace_net_vector_search_smoke_v1_quality.py ...

When Python executes a file from scripts/, sys.path points at scripts/ rather
than the repository root. Add the repo root explicitly so imports from tiff/
work in Git Bash, PowerShell, and CI.
"""

from __future__ import annotations

import sys
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parents[2]
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from tiff.trace_net_vector_search_smoke_v1 import quality_main


if __name__ == "__main__":
    raise SystemExit(quality_main())
