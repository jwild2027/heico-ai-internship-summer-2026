...
"$PYTHON_BIN" -B scripts/maintenance/s3_graph_store/check_trace_net_nha_phase9_release_v1.py \
...
"$PYTHON_BIN" -B scripts/benchmark/s3_graph_store/run_trace_net_nha_phase11_shadow_http_smoke_v1.py \
...
rm -rf "$LIVE20_DIR"
"$PYTHON_BIN" -B scripts/benchmark/s3_graph_store/run_trace_net_nha_phase10_live20_v1.py \
  --phase4-dir "$RELEASE_DIR" \
  --base-url "$BASE_URL" \
  --api-key "$API_KEY" \
  --model "$MODEL" \
