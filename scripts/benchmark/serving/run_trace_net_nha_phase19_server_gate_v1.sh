...
rollback_on_failure() {
  status=$?
  bash scripts/operations/launch_trace_net_nha_phase19_stack_v1.sh rollback || true
  exit "$status"
}
trap rollback_on_failure ERR

bash scripts/operations/launch_trace_net_nha_phase19_stack_v1.sh promote
...
"$PYTHON" -B scripts/operations/graph/run_trace_net_nha_phase19_unified8131_gate_v1.py \
...
"$PYTHON" -B scripts/maintenance/graph/check_trace_net_nha_phase19_unified8131_gate_v1.py \
  --output-dir "$RUN_ROOT/mixed12" \
  --strict
...
echo "rollback_command=bash scripts/operations/launch_trace_net_nha_phase19_stack_v1.sh rollback"
