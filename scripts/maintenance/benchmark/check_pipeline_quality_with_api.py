...
def main() -> int:
    ...
    if not args.skip_refresh:
        for cmd in [
            ...
            [sys.executable, "scripts/maintenance/core/check_tiff_storage_adapters.py", "--write-json"],
            [sys.executable, "scripts/maintenance/validation/check_api_adapter_quality.py", "--write-json"],
            [sys.executable, "scripts/maintenance/validation/refresh_api_adapter_quality_summary.py"],
        ]:
            rc = run(cmd)
            if rc != 0:
                return rc

    pipeline_cmd = [sys.executable, "scripts/maintenance/validation/check_pipeline_quality.py"]
    for flag in [
