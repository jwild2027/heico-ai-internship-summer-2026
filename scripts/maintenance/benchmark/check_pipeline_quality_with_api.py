...
def main() -> int:
    ...
    if not args.skip_refresh:
        for cmd in [
            [sys.executable, "scripts/maintenance/serving/check_tiff_api_ready.py", "--write-json"],
            [sys.executable, "scripts/maintenance/ingestion/check_tiff_storage_adapters.py", "--write-json"],
            [sys.executable, "scripts/maintenance/benchmark/check_api_adapter_quality.py", "--write-json"],
            [sys.executable, "scripts/maintenance/serving/refresh_api_adapter_quality_summary.py"],
        ]:
            rc = run(cmd)
            if rc != 0:
                return rc

    pipeline_cmd = [sys.executable, "scripts/maintenance/benchmark/check_pipeline_quality.py"]
    for flag in [
