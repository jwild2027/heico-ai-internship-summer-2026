"""scripts/ingest_batch.py — Batch ingest all PDFs in a directory into the RAG DB.

Skips PDFs that are already in the DB (same checksum).
Logs per-file results and prints a summary at the end.

Usage:
    python scripts/ingest_batch.py --dir data/nist_pdfs --db-path rag.db
    python scripts/ingest_batch.py --dir data/nist_pdfs --db-path rag.db --workers 1
"""
