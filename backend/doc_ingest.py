...
def extract_pages(pdf_path: Path) -> list[dict[str, Any]]:
    try:
        ...
        raise ModuleNotFoundError(
            "pypdf is not installed in the active environment. "
...
def run_ingest(
    ...
    run_ingest_with_pages(pages, pdf_path, output, model, chunk_words, overlap, prompt)


def run_ingest_with_pages(
    pages: list[dict[str, Any]],
    source_pdf: Path,
    output: Path | str,
    model: str,
    ...
    """Ingest entrypoint when pages are already extracted by another scraper."""
    ...
    if not chunks:
        raise RuntimeError(f"No extractable text found in {source_pdf}")
