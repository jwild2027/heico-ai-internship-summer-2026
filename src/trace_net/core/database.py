...
class RAGDatabase:
    ...
    def upsert_document(self, filepath: Path, *, doc_id: Optional[str] = None) -> tuple[str, bool]:
        ...
        with self._conn:
            self._conn.execute(
                "INSERT INTO documents (id, filename, filepath, pdf_checksum, status) VALUES (?, ?, ?, ?, 'pending')",
    ...
    def clear_document_content(self, doc_id: str) -> dict[str, int]:
        """Delete pages, page_texts, images, and chunks for a doc inside a single transaction."""
        ...
        with self._conn:
            ...
            counts["page_texts"] = self._conn.execute(
                "DELETE FROM page_texts WHERE page_id IN (SELECT id FROM pages WHERE doc_id=?)",
                (doc_id,),
            ).rowcount
            counts["images"] = self._conn.execute(
                "DELETE FROM images WHERE page_id IN (SELECT id FROM pages WHERE doc_id=?)",
                (doc_id,),
            ).rowcount
    ...
    def delete_document(self, doc_id: str) -> None:
        with self._conn:
            chunk_ids = [r["id"] for r in self._conn.execute(
            ...
            page_ids = [r["id"] for r in self._conn.execute(
                "SELECT id FROM pages WHERE doc_id=?", (doc_id,)
            ).fetchall()]
            if page_ids:
                placeholders = ",".join("?" * len(page_ids))
                self._conn.execute(f"DELETE FROM page_texts WHERE page_id IN ({placeholders})", page_ids)
                self._conn.execute(f"DELETE FROM images WHERE page_id IN ({placeholders})", page_ids)
                self._conn.execute(f"DELETE FROM pages WHERE id IN ({placeholders})", page_ids)

            self._conn.execute("DELETE FROM ingestion_runs WHERE doc_id=?", (doc_id,))
    ...
    def start_ingestion_run(self, doc_id: str, config: dict[str, Any]) -> str:
        ...
        with self._conn:
            self._conn.execute(
                "INSERT INTO ingestion_runs (id, doc_id, status, config_json) VALUES (?, ?, 'running', ?)",
    ...
    def finish_ingestion_run(self, run_id: str, *, error: Optional[str] = None) -> None:
        ...
        with self._conn:
            self._conn.execute(
                "UPDATE ingestion_runs SET status=?, finished_at=?, error=? WHERE id=?",
    ...
    def insert_page(self, doc_id: str, run_id: str, page_number: int, *,
                    width_pt: Optional[float] = None, height_pt: Optional[float] = None) -> str:
        page_id = _new_id()
        with self._conn:
            self._conn.execute(
                "INSERT INTO pages (id, doc_id, run_id, page_number, width_pt, height_pt) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    def insert_page_text(self, page_id: str, strategy: str, text: str, *,
                         quality_score: Optional[float] = None,
                         confidence: Optional[float] = None,
                         is_selected: bool = False) -> str:
        pt_id = _new_id()
        words = len(text.split()) if text else 0
        with self._conn:
            self._conn.execute(
                """INSERT INTO page_texts
                    (id, page_id, strategy, text, quality_score, confidence, char_count, word_count, is_selected)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (pt_id, page_id, strategy, text, quality_score, confidence, len(text), words, 1 if is_selected else 0),
            )
        return pt_id

    def set_selected_page_text(self, page_id: str, page_text_id: str) -> None:
        with self._conn:
            self._conn.execute("UPDATE page_texts SET is_selected=0 WHERE page_id=?", (page_id,))
            self._conn.execute("UPDATE page_texts SET is_selected=1 WHERE id=?", (page_text_id,))

    def get_selected_page_text(self, page_id: str) -> Optional[dict[str, Any]]:
        row = self._conn.execute(
            "SELECT * FROM page_texts WHERE page_id=? AND is_selected=1", (page_id,)
        ).fetchone()
        return dict(row) if row else None

    def get_all_page_texts(self, page_id: str) -> list[dict[str, Any]]:
        rows = self._conn.execute(
            "SELECT * FROM page_texts WHERE page_id=? ORDER BY created_at", (page_id,)
        ).fetchall()
        return [dict(r) for r in rows]
    ...
    def insert_image(self, page_id: str, image_type: str, path: str | Path, *,
        ...
        with self._conn:
            self._conn.execute(
                "INSERT INTO images (id, page_id, image_type, path, width_px, height_px, dpi, checksum) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (img_id, page_id, image_type, str(path), width_px, height_px, dpi, checksum),
            )
        return img_id
    ...
    def insert_chunk(self, doc_id: str, page_id: str, page_text_id: str, run_id: str,
                     chunk_index: int, text: str, *, title: Optional[str] = None,
                     char_start: Optional[int] = None, char_end: Optional[int] = None,
                     word_count: Optional[int] = None, token_count: Optional[int] = None,
                     page_start: Optional[int] = None, page_end: Optional[int] = None,
                     chunker_version: str = "semantic_v1", config: Optional[dict[str, Any]] = None,
                     strategy: str = "flat", level: str = "flat", parent_id: Optional[str] = None,
                     explicit_id: Optional[str] = None) -> str:
        if explicit_id:
            ...
            self._conn.execute(
                """INSERT INTO chunks
                   ...
                   ON CONFLICT(id) DO UPDATE SET
                    text=excluded.text, title=excluded.title, word_count=excluded.word_count,
                    page_start=excluded.page_start, page_end=excluded.page_end,
                    run_id=excluded.run_id, strategy=excluded.strategy,
                    level=excluded.level, parent_id=excluded.parent_id, embedded=0""",
                (chunk_id, doc_id, page_id, page_text_id, run_id, chunk_index,
                 text, title, char_start, char_end, wc, token_count,
                 page_start, page_end, chunker_version, strategy, level,
                 parent_id, json.dumps(config) if config else None),
            )
        return chunk_id

    def get_parent_chunk(self, child_chunk_id: str) -> Optional[dict[str, Any]]:
        row = self._conn.execute(
            """SELECT p.* FROM chunks p JOIN chunks c ON c.parent_id = p.id
    ...
    def search_chunks_by_title(self, search_term: str, level: str = "child") -> list[dict[str, Any]]:
        """Search chunks for an exact control ID match in title OR text.
        Uses word-boundary patterns to avoid AC-2 matching AC-20, AC-21 etc.
        """
        # SQLite LIKE doesn't support word boundaries, so we use multiple patterns
        # to enforce that the term is followed by space, end-of-string, or non-alphanumeric
        patterns = [
            f"% {search_term} %",   # surrounded by spaces
            f"{search_term} %",     # at start, followed by space
            f"% {search_term}",     # at end, preceded by space
            f"%({search_term})%",   # in parentheses
            f"%{search_term}.%",    # followed by period
            f"%{search_term},%",    # followed by comma
            f"%{search_term}:%",    # followed by colon
        ]

        # Build OR clause for title search
        title_conditions = " OR ".join(["title LIKE ?"] * len(patterns))
        text_conditions  = " OR ".join(["text LIKE ?"]  * len(patterns))

        rows = self._conn.execute(
            f"""SELECT * FROM chunks
                WHERE level = ?
                    AND (({title_conditions}) OR ({text_conditions}))
                ORDER BY
                    CASE WHEN ({title_conditions}) THEN 0 ELSE 1 END,
                    page_start
                LIMIT 20""",
            (level, *patterns, *patterns, *patterns),
        ).fetchall()
        return [dict(r) for r in rows]
    ...
    def mark_chunks_embedded(self, chunk_ids: list[str]) -> None:
        with self._conn:
            placeholders = ",".join("?" * len(chunk_ids))
            self._conn.execute(
                f"UPDATE chunks SET embedded=1 WHERE id IN ({placeholders})", chunk_ids
            )

    def get_unembedded_chunks(self, doc_id: Optional[str] = None) -> list[dict[str, Any]]:
        if doc_id:
            rows = self._conn.execute(
                "SELECT * FROM chunks WHERE embedded=0 AND doc_id=? ORDER BY chunk_index", (doc_id,)
            ).fetchall()
        else:
            rows = self._conn.execute(
    ...
    def log_retrieval(self, query_text: str, chunk_ids: list[str], distances: list[float], *,
                      query_embedding_model: Optional[str] = None, top_k: Optional[int] = None,
                      fetch_k: Optional[int] = None, rerank_method: Optional[str] = None,
                      latency_ms: Optional[float] = None, llm_model: Optional[str] = None,
                      llm_response: Optional[str] = None, grounded: Optional[bool] = None) -> str:
        log_id = _new_id()
        with self._conn:
            self._conn.execute(
                ...
                (log_id, query_text, query_embedding_model, top_k, fetch_k,
                 rerank_method, json.dumps(chunk_ids), json.dumps(distances),
    ...
    def status(self) -> dict[str, Any]:
        ...
        embedded_count= self._conn.execute("SELECT COUNT(*) FROM chunks WHERE embedded=1").fetchone()[0]
        ...
        docs = self._conn.execute(
            """SELECT d.filename, d.status, d.page_count,
                      COUNT(c.id) AS chunks, SUM(c.embedded) AS embedded
