...
VERSION = "v34.2"
MODULE = "trace_net_e2e_image_visual_observer_route_v34_2"
MODEL_ID = "trace-net-e2e-image-ocr-opencv-fusion-llava-v34-2"
...
def _synthetic_ocr_text_card(image_id: str, visual_type: str | None, query: str) -> Dict[str, Any]:
    ...
    return {
        "ocr_card_id": _stable_id("ocr_text_v34_2", image_id + "|" + low),
        "image_id": image_id,
...
def _ocr_text_card_from_bytes(image_id: str, image_bytes: bytes | None) -> Dict[str, Any]:
    ...
    return {
        "ocr_card_id": _stable_id("ocr_text_v34_2", image_id + "|" + hashlib.sha1((image_bytes or b'')).hexdigest()[:12]),
        "image_id": image_id,
...
def _synthetic_opencv_layout_card(image_id: str, visual_type: str | None, query: str) -> Dict[str, Any]:
    ...
    return {
        "layout_card_id": _stable_id("opencv_layout_v34_2", image_id + "|" + low),
        "image_id": image_id,
...
def _opencv_layout_card_from_bytes(image_id: str, image_bytes: bytes | None) -> Dict[str, Any]:
    ...
    if image_bytes:
        try:
            ...
            error = f"{type(exc).__name__}: {exc}"
    return {
        "layout_card_id": _stable_id("opencv_layout_v34_2", image_id + "|" + hashlib.sha1((image_bytes or b'')).hexdigest()[:12]),
        "image_id": image_id,
...
def _visual_crag(package: Mapping[str, Any], self_rag: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if not has_image:
        ...
        route = "run_ocr_opencv_fusion_or_request_clearer_image"
...
def _build_diagram_draft_card(query: str, package: Mapping[str, Any]) -> Dict[str, Any] | None:
    ...
    v34.2 favors OCR/OpenCV-derived regions over raw LLaVA prose. LLaVA can
    ...
    def add_node(node_id: str, label: str, kind: str, confidence: str = "medium", source: str = "ocr_opencv_fusion_guidance") -> None:
    ...
    return {
        "diagram_draft_id": _stable_id("diagram_draft_v34_2", query + json.dumps(nodes, sort_keys=True)),
...
def _visual_safe_answer(package: Mapping[str, Any]) -> str:
    ...
    parts = [
        f"TRACE-Net built an OCR/OpenCV-fused visual guidance package for {image_count} image(s).",
    ...
    parts.append("LLaVA observations are retained as guidance only; OCR/OpenCV grounding is used before text/region claims are surfaced.")
    if unconfirmed:
    ...
    if diagram_cards and _wants_diagram_draft(query):
        ...
        return (
            base
            + "\n\nDiagram draft generated from OCR/OpenCV-fused visual package (guidance only):"
            + "\n\n```mermaid\n"
            + mermaid
            + "\n```\n\n"
...
def build_visual_package(
    *,
    ...
    package: Dict[str, Any] = {
        "package_id": _stable_id("tracenet_visual_package_v34_2", user_query + json.dumps(image_quality_cards, sort_keys=True)),
        "version": VERSION,
        "module": MODULE,
        ...
        "ocr_opencv_fusion_applied": bool(ocr_text_cards or opencv_layout_cards),
...
def build_report(
    *,
    ...
    report_path = outdir / "trace_net_e2e_image_visual_observer_route_v34_2.json"
    records_jsonl_path = outdir / "trace_net_e2e_image_visual_observer_route_records_v34_2.jsonl"
    inspect_md_path = outdir / "trace_net_e2e_image_visual_observer_route_v34_2.md"
...
def serve_endpoint(
    *,
    ...
    print(f"Serving TRACE-Net image visual observer + OCR/OpenCV fusion route v34.2 on http://{host}:{port}/v1")
