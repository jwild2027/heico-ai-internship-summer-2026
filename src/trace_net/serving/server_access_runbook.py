...
def build_runbook_steps(
    *,
    ...
    ocr_depth = (
        "python scripts/maintenance/ocr/audit_ocr_depth.py "
        f"--root {server_root} --max-files {max_files} --write-json "
        "--json-output local_data/ocr/real_server_ocr_depth_sample.json"
    )
    batch_audit = (
        "python scripts/maintenance/serving/audit_document_batch.py "
        f"--root {server_root} --max-files {max_files} --write-json"
    )
    pilot = (
        "python scripts/operations/ocr/run_ocr_pilot.py "
