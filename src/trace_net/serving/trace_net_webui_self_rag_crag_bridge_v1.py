...
def _visual_context_bridge_counts(payload: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    if not payload:
        return {
            "quality_status": None,
            "visual_context_card_count": 0,
            "review_only_visual_context_excluded_count": 0,
            "included_pages": [],
            "included_canonical_page_numbers": [],
        }
    summary = _safe_summary(payload)
    records = _records(payload)
    return {
        "quality_status": payload.get("quality_status"),
        "visual_context_card_count": int(summary.get("visual_context_card_count") or len(records) or 0),
        "review_only_visual_context_excluded_count": int(summary.get("review_only_visual_context_excluded_count") or 0),
        "included_pages": list(summary.get("included_pages") or [record.get("page_id") for record in records if record.get("page_id")]),
        "included_canonical_page_numbers": list(summary.get("included_canonical_page_numbers") or []),
    }


def _webui_visual_context_bridge_row(path: Optional[Path], payload: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    path_state = _path_status(path)
    counts = _visual_context_bridge_counts(payload)
    quality = counts.get("quality_status")
    card_count = int(counts.get("visual_context_card_count") or 0)
    excluded_count = int(counts.get("review_only_visual_context_excluded_count") or 0)
    if path_state == "input_missing":
        status = "input_missing"
        reason = "configured path for webui_visual_context_bridge does not exist"
    elif path_state == "not_configured":
        status = "not_configured"
        reason = "no path configured for webui_visual_context_bridge"
    elif quality != "PASS":
        status = "failed"
        reason = f"webui_visual_context_bridge quality_status={quality}"
    elif card_count > 0:
        status = "used"
        reason = f"loaded {card_count} safe visual context card(s); excluded {excluded_count} review-only visual card(s)"
    else:
        status = "available_not_used"
        reason = f"bridge exists and passed, but no safe visual context cards were available; excluded {excluded_count} review-only card(s)"
    return _stage_row(
        tool_id="webui_visual_context_bridge",
        label="webui visual context bridge",
        status=status,
        reason=reason,
        path=path,
        count=card_count,
        quality_status=str(quality) if quality is not None else None,
    )


def _artifact_tool_rows(
    context_pack_payload: Mapping[str, Any],
    input_paths: Mapping[str, Optional[Path]],
    *,
    webui_visual_context_bridge_payload: Optional[Mapping[str, Any]] = None,
    webui_visual_context_bridge_path: Optional[Path] = None,
) -> List[Dict[str, Any]]:
    summary = _safe_summary(context_pack_payload)
    artifact_counts = summary.get("artifact_record_counts") or {}
    visual_counts = _visual_context_bridge_counts(webui_visual_context_bridge_payload)
    visual_card_count = int(visual_counts.get("visual_context_card_count") or 0)
    visual_excluded_count = int(visual_counts.get("review_only_visual_context_excluded_count") or 0)
    ...
    for tool_id, artifact_name in ARTIFACT_TOOL_KEYS.items():
        ...
        if tool_id == "visual_image_route" and visual_card_count > 0 and webui_visual_context_bridge_payload and webui_visual_context_bridge_payload.get("quality_status") == "PASS":
            status = "used"
            reason = (
                f"safe WebUI visual context bridge supplied {visual_card_count} OCR-supported visual card(s); "
                f"{visual_excluded_count} review-only visual card(s) excluded"
            )
            path = webui_visual_context_bridge_path or path
            count = visual_card_count
        elif count > 0:
            status = "used"
    ...
    rows.append(_webui_visual_context_bridge_row(webui_visual_context_bridge_path, webui_visual_context_bridge_payload))
    rows.append(
        _stage_row(
...
def build_webui_self_rag_crag_bridge(
    *,
    ...
    webui_visual_context_bridge: Optional[Path] = None,
    ...
    # Stage builders may write their own report files without creating the
    # nested output directory first. Pre-create every stage directory so the
    # bridge works from a clean output directory and from WebUI wrapper
    # preflight paths.
    for directory in (planner_dir, blueprint_dir, pack_dir, self_rag_dir, crag_dir):
    ...
    webui_visual_context_bridge_payload: Optional[Dict[str, Any]] = None
    if webui_visual_context_bridge is not None and webui_visual_context_bridge.exists():
        webui_visual_context_bridge_payload = _read_json(webui_visual_context_bridge)

    stage_payloads = [planner_payload, blueprint_payload, pack_payload, self_rag_payload, crag_payload]
    if webui_visual_context_bridge_payload is not None:
        stage_payloads.append(webui_visual_context_bridge_payload)
    stage_paths = {
    ...
    input_paths = {
        ...
        "graph_leiden": leiden_communities,
        "visual_image_route": image_visual_observer,
        "webui_visual_context_bridge": webui_visual_context_bridge,
    }
    rows.extend(
        _artifact_tool_rows(
            pack_payload,
            input_paths,
            webui_visual_context_bridge_payload=webui_visual_context_bridge_payload,
            webui_visual_context_bridge_path=webui_visual_context_bridge,
        )
    )
    rows.append(
        _stage_row(
    ...
    summary: Dict[str, Any] = {
        ...
        "webui_visual_context_bridge_used": statuses.get("webui_visual_context_bridge") == "used",
        "visual_image_route_used": statuses.get("visual_image_route") == "used",
        "webui_visual_context_bridge_quality_status": _visual_context_bridge_counts(webui_visual_context_bridge_payload).get("quality_status"),
        "visual_context_card_count": int(_visual_context_bridge_counts(webui_visual_context_bridge_payload).get("visual_context_card_count") or 0),
        "review_only_visual_context_excluded_count": int(_visual_context_bridge_counts(webui_visual_context_bridge_payload).get("review_only_visual_context_excluded_count") or 0),
        "visual_context_included_pages": _visual_context_bridge_counts(webui_visual_context_bridge_payload).get("included_pages") or [],
        "visual_context_included_canonical_page_numbers": _visual_context_bridge_counts(webui_visual_context_bridge_payload).get("included_canonical_page_numbers") or [],
        **safety,
    }
    ...
    payload: Dict[str, Any] = {
        ...
        "webui_visual_context_cards": _records(webui_visual_context_bridge_payload or {}),
        "thresholds": {
            "max_records_per_slot": max_records_per_slot,
            "min_high_signal_capsules": min_high_signal_capsules,
            "min_evidence_strength_score": min_evidence_strength_score,
...
def _write_markdown(path: Path, payload: Mapping[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Visual context cards: `{summary.get('visual_context_card_count')}`",
        f"- Review-only visual cards excluded: `{summary.get('review_only_visual_context_excluded_count')}`",
        "",
        "## Checklist",
        "",
        "```text",
...
def check_webui_self_rag_crag_bridge_quality(
    *,
    ...
    if require_webui_visual_context_bridge_used:
        fail_if(statuses.get("webui_visual_context_bridge") != "used", "WebUI visual context bridge was not used")
        fail_if(statuses.get("visual_image_route") != "used", "visual/image route was not used")
    if min_visual_context_cards:
        fail_if(int(summary.get("visual_context_card_count") or 0) < min_visual_context_cards, "visual context card count below minimum")
...
def main_build(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--webui-visual-context-bridge")
