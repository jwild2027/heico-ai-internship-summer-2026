...
class LocalTraceStore:
    ...
    def trace_part(self, part_number: str, limit: int = 8) -> dict[str, Any]:
        args = [sys.executable, "scripts/maintenance/s3_graph_store/trace_document_graph.py", "--part", part_number, "--limit", str(limit), "--strict", "--write-json"]
        result = _run_command(args)
        report = self._read_trace_report()
        return {"ok": result["returncode"] == 0, "trace": report, "command_result": result}

    def trace_page(self, page_id: str) -> dict[str, Any]:
        args = [sys.executable, "scripts/maintenance/s3_graph_store/trace_document_graph.py", "--page", page_id, "--strict", "--write-json"]
        result = _run_command(args)
        report = self._read_trace_report()
        return {"ok": result["returncode"] == 0, "trace": report, "command_result": result}

    def trace_vector(self, page_id: str, chunk_id: str | None = None, score: float | None = None) -> dict[str, Any]:
        args = [sys.executable, "scripts/maintenance/s3_graph_store/trace_document_graph.py", "--vector-page", page_id, "--strict", "--write-json"]
        if chunk_id:
...
class LocalAnswerStore:
    ...
    def ask(self, question: str, timeout_seconds: int = 120) -> dict[str, Any]:
        args = [sys.executable, "scripts/operations/ingestion/ask_tiff_rag.py", "--config", str(self.config_path), question]
