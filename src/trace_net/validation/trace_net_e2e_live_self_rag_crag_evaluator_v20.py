...
def _get_evidence_records(pack: Mapping[str, Any]) -> List[Any]:
    candidates = [
        ...
        _nested(pack, ["evidence_box", "items"]),
        _nested(pack, ["evidence_box", "records"]),
        _nested(pack, ["source_truth_evidence_box", "items"]),
    ...
    if count < 0:
        count = _as_int(
            _nested(pack, ["evidence_box", "source_truth_evidence_count"], None),
            -1,
        )
    if count < 0:
        count = _as_int(_nested(pack, ["evidence_box", "item_count"], 0))
