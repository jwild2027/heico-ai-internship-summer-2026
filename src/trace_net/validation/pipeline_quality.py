...
def check_pipeline_manifest(
    ...
    *,
    ...
    _add_check(
        ...
        else "OCR coverage audit summary is missing; run the pipeline or scripts/maintenance/ocr/audit_ocr_coverage.py --write-json.",
    )
    _add_check(
        checks,
    ...
    _add_check(
        ...
        else "Source-package traceability is optional unless --require-source-package-traceability is set; run scripts/maintenance/ingestion/audit_source_package_traceability.py --write-json and scripts/maintenance/benchmark/check_source_package_quality.py --write-json.",
    )
    ...
    _add_check(
        ...
        if graph_present
        else "Document graph quality artifacts are missing; run scripts/build/graph/export_document_organization_graph.py --strict and scripts/maintenance/benchmark/check_document_graph_quality.py --write-json.",
    )
    ...
    _add_check(
        ...
        else "Incremental smoke summary is not required yet; run scripts/benchmark/smoke_test_incremental_changed_page.py --write-json to add it.",
    )
    if incremental_present:
        _add_check(
            checks,
