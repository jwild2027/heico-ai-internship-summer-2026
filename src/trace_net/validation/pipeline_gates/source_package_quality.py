...
``scripts/maintenance/ingestion/audit_source_package_traceability.py`` and turns it into a compact
...
def build_source_package_quality_result(
    ...
    *,
    ...
    _add_check(
        ...
        if present
        else f"Source-package traceability report is missing at {traceability_json}; run scripts/maintenance/ingestion/audit_source_package_traceability.py --write-json.",
    )
    _add_check(
        checks,
