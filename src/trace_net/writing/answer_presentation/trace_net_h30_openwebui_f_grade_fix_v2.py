...
def _merge_validation(
    *,
    ...
    acceptance_basis = "technical_validator_and_public_contract"
    deterministic_exact_ipl_match = False

    if safe_exact_ipl:
        # Accept only output generated by this module's deterministic, proof-
        # gated IPL renderer. The legacy technical validator remains preserved
        # for audit, but it cannot reject a truthful partial result merely
        # because unavailable fields are explicitly marked "not proven".
        expected_content = render_table_field_answer(query, result)
        deterministic_exact_ipl_match = (
            content.strip() == expected_content.strip()
        )
        accepted = bool(
            contract.get("accepted")
            and deterministic_exact_ipl_match
            and not contains_internal_diagnostic(content)
        )
        acceptance_basis = "deterministic_exact_ipl_field_status"
        if accepted:
            final_failures = []
        else:
            final_failures = list(failures)
            if not deterministic_exact_ipl_match:
                final_failures.append(
                    "deterministic_exact_ipl_field_status_mismatch"
                )
            if contains_internal_diagnostic(content):
                final_failures.append("internal_diagnostic_present")
        ...
        acceptance_basis = "deterministic_safe_general_contract"
    else:
    ...
    return {
        **base,
        ...
        "deterministic_exact_ipl_match": deterministic_exact_ipl_match,
    }
...
def install_openwebui_f_grade_fix(module: MutableMapping[str, Any]) -> None:
    ...
    def health_openwebui_f_grade_fix(self: Any) -> Dict[str, Any]:
        ...
        result.update({
            ...
            "exact_ipl_deterministic_validation_acceptance_enabled": True,
            "exact_ipl_legacy_validator_preserved_for_audit": True,
            "exact_ipl_validator_can_promote_guidance_to_proof": False,
