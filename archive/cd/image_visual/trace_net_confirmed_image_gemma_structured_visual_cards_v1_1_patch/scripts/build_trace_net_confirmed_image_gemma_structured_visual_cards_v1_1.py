...
MODULE_NAME = "trace_net_confirmed_image_gemma_structured_visual_cards_v1_1"
STATUS_BUILT = "TRACE_NET_CONFIRMED_IMAGE_GEMMA_STRUCTURED_VISUAL_CARDS_V1_1_BUILT"
...
- Do not use proof words such as "confirm", "confirmation", "verify", "prove", "establish", "certify", or "validate" for visual facts.
...
def deterministic_structured_card(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "evidence_use": "Use this card to retrieve likely relevant visual/diagram pages. This is not final proof for exact text, fit, interchangeability, effectivity, approval, eligibility, or installation.",
...
def is_concrete_callout(value: str) -> bool:
    text = compact_ws(value, limit=200)
    low = text.lower()
    if not text:
        return False
    if low in GENERIC_CALLOUT_VALUES:
        return False
    if len(text.split()) > 6 and not re.search(r"\b(item|callout|fig(?:ure)?|part|no\.?|number)\b", low):
        return False
    return True


def normalize_callouts(values: Any, *, limit: int = 60) -> List[str]:
    return stable_unique([v for v in clean_list(values, limit=limit * 2) if is_concrete_callout(v)], limit=limit)


def contains_proof_word(value: Any) -> bool:
    return bool(PROOF_WORD_RE.search(compact_ws(value, limit=2000)))


def sanitize_evidence_use(value: Any) -> str:
    text = compact_ws(value, limit=1200)
    if not text:
...
def build_summary(
    ...
    *,
    ...
    for r in records:
        card = r.get("structured_visual_card") or {}
        if contains_proof_word(card.get("evidence_use")):
            proof_word_evidence_use_count += 1
        for c in card.get("visible_callouts") or []:
            if compact_ws(c, limit=200).lower() in GENERIC_CALLOUT_VALUES:
                generic_callout_count += 1
        for fig in card.get("figure_refs") or []:
            if not re.fullmatch(r"figure [0-9]{1,4}[a-z]?(?: sheet [0-9]{1,3})?", compact_ws(fig, limit=200).lower()):
                non_figure_ref_count += 1
    ...
    if proof_word_evidence_use_count:
        quality_status = "FAIL"
        quality_issues.append(f"proof_word_evidence_use_count:{proof_word_evidence_use_count}")
    if generic_callout_count:
        quality_status = "FAIL"
        quality_issues.append(f"generic_callout_count:{generic_callout_count}")
    if non_figure_ref_count:
        quality_status = "FAIL"
        quality_issues.append(f"non_figure_ref_count:{non_figure_ref_count}")
    ...
    return {
        ...
        "proof_word_evidence_use_count": proof_word_evidence_use_count,
        "generic_callout_count": generic_callout_count,
        "non_figure_ref_count": non_figure_ref_count,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    write_jsonl(out_dir / "trace_net_confirmed_image_gemma_structured_visual_cards_v1_1.jsonl", records)
    write_jsonl(out_dir / "trace_net_confirmed_image_gemma_structured_visual_retrieval_documents_v1_1.jsonl", retrieval_docs)
    ...
    (out_dir / "trace_net_confirmed_image_gemma_structured_visual_cards_v1_1_report.txt").write_text(
        "\n".join(report_lines) + "\n",
        encoding="utf-8",
    )
    ...
    for key in [
        ...
        "proof_word_evidence_use_count",
        "generic_callout_count",
        "non_figure_ref_count",
