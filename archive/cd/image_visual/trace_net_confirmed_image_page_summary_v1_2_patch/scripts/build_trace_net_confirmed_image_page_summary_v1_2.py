...
MODULE_NAME = "trace_net_confirmed_image_page_summary_v1_2"
STATUS_BUILT = "TRACE_NET_CONFIRMED_IMAGE_PAGE_SUMMARY_V1_2_BUILT"
QUALITY_PASS = "PASS"
QUALITY_FAIL = "FAIL"
...
def collect_nested_texts(value: Any, *, limit: int = 2000, max_items: int = 300) -> List[str]:
    """Collect strings from nested JSON-like docs.

    The gated visual adapter has changed shape across patches. Figure refs and
    subjects may live under top-level fields, identifiers, context snippets,
    visual summaries, source text, or nested route/context payloads. This
    recursive collector gives the summary card a stable way to harvest text
    signals without depending on one artifact schema.
    """
    out: List[str] = []
    ...
    def walk(v: Any) -> None:
        ...
        if v is None:
            return
        if isinstance(v, str):
            text = compact_ws(v, limit=limit)
            if text:
                out.append(text)
            return
        if isinstance(v, (int, float)):
            # Keep tiny numeric fields out of recursive text harvesting. Bare
            # callout/page numbers are too noisy unless they came from explicit
            # figure_refs.
            return
        if isinstance(v, dict):
            for key, child in v.items():
                if len(out) >= max_items:
                    return
                # Include key names for figure/title/subject-bearing fields.
                if isinstance(child, str) and str(key).lower() in {
                    ...
                    out.append(compact_ws(f"{key}: {child}", limit=limit))
                walk(child)
            return
    ...
    walk(value)
    return stable_unique(out, limit=max_items)


def values_for_keys(value: Any, key_names: Sequence[str], *, max_items: int = 80) -> List[Any]:
            ...
            for key, child in v.items():
                if str(key).lower() in targets:
                    if isinstance(child, list):
                        out.extend(child[: max_items - len(out)])
                    else:
                        out.append(child)
                walk(child)
...
def figure_refs_from_texts(texts: Iterable[Any]) -> List[str]:
    refs: List[str] = []
    for value in texts:
        text = compact_ws(value, limit=2000)
        refs.extend(m.group(0).lower() for m in FIG_RE.finditer(text))
    return stable_unique(refs, limit=40)
...
def is_real_part_number_candidate(value: Any) -> bool:
    text = compact_ws(value, limit=200).strip().upper()
    if not text:
        return False
    if text in NOISE_PART_TOKENS:
        return False
    if any(token in text for token in ("OCR", "VISUAL", "REVIEW", "SUMMARY", "TABLE", "FIGURE-ITEM")):
        return False
    if re.fullmatch(r"\d{2}-\d{2}-\d{2}", text):
        return False
    if re.fullmatch(r"(25|21|00|06|12|377)", text):
        return False
    if re.fullmatch(r"\d{3}-\d{5}-\d{3}(?:/\d{3})?", text):
        return True
    if re.fullmatch(r"\d{3}-\d{5}-\d{1,2}", text):
        return False
    if re.fullmatch(r"MS\d{4,6}-[A-Z]\d+[A-Z0-9-]*", text):
        return True
    if re.fullmatch(r"[A-Z]{1,6}\d{1,6}[A-Z]{0,4}-[A-Z0-9]{1,8}(?:-[A-Z0-9]{1,8})?", text):
        return True
    return False


def filter_real_part_numbers(values: Iterable[Any], *, limit: int = 40) -> List[str]:
    return stable_unique([compact_ws(v, limit=200).upper() for v in values if is_real_part_number_candidate(v)], limit=limit)


def cleaned_llava_payload(llava_observation: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    if not isinstance(llava_observation, dict):
        return {}
    clean = llava_observation.get("llava_observation_cleaned")
    return clean if isinstance(clean, dict) and clean.get("parsed") else {}


def llava_visual_observation_text(llava_clean: Dict[str, Any], raw_fallback: Any = "") -> str:
    parts: List[str] = []
    subject = compact_ws(llava_clean.get("diagram_subject_guess"), limit=500)
    if subject and subject.lower() != "unknown":
        parts.append(f"subject: {subject}")
    layout = compact_ws(llava_clean.get("visual_layout_description"), limit=1200)
    if layout:
        parts.append(f"layout: {layout}")
    figure = compact_ws(llava_clean.get("figure_title_or_sheet_text_if_clearly_visible"), limit=300)
    if figure:
        parts.append(f"figure: {figure}")
    callouts = llava_clean.get("visible_callouts_or_labels_cleaned") or []
    if callouts:
        parts.append("cleaned visible callouts: " + ", ".join(stable_unique(callouts, limit=20)))
    uncertainty = compact_ws(llava_clean.get("visual_uncertainty"), limit=800)
    if uncertainty:
        parts.append(f"uncertainty: {uncertainty}")
    if parts:
        return " | ".join(parts)
    return compact_ws(raw_fallback, limit=1200)


def llava_subject_hint(llava_clean: Dict[str, Any]) -> str:
    subject = compact_ws(llava_clean.get("diagram_subject_guess"), limit=300)
    if not subject or subject.lower() == "unknown":
        return ""
    return subject


def llava_figure_refs(llava_clean: Dict[str, Any]) -> List[str]:
    figure = compact_ws(llava_clean.get("figure_title_or_sheet_text_if_clearly_visible"), limit=300)
    values: List[str] = []
    if figure:
        values.append(figure)
    values.extend(llava_clean.get("visible_callouts_or_labels_cleaned") or [])
    return figure_refs_from_texts(values)


def llava_callouts(llava_clean: Dict[str, Any]) -> List[str]:
    return stable_unique(llava_clean.get("visible_callouts_or_labels_cleaned") or [], limit=80)
...
def infer_subject(
    ...
    *,
    ...
    llava_subject = compact_ws(llava_subject, limit=180)
    if llava_subject and llava_subject.lower() not in {"unknown", "parts_diagram_or_illustrated_parts_list"}:
        return llava_subject
    ...
    candidates = [
        ...
        doc.get("title"),
        doc.get("page_title"),
        *values_for_keys(doc, ("nomenclature", "subject", "title", "page_title", "caption")),
    ]
    for c in candidates:
        ...
        if not text:
            continue
        if low in {"unknown", "parts_diagram_or_illustrated_parts_list"}:
            continue
        if "parts_diagram_or_illustrated_parts_list" in low:
            continue
    ...
    # Never use raw/composed LLaVA text as subject. It belongs in visual_observations.
...
def find_text_values(doc: Dict[str, Any]) -> List[str]:
    ...
    for key in (
        ...
        if isinstance(value, str):
            ...
            explicit.extend(compact_ws(v, limit=500) for v in value)

    nested = collect_nested_texts(doc)
    return stable_unique([*explicit, *nested], limit=300)
...
def build_summary_card(
    ...
    *,
    ...
    llava_clean = cleaned_llava_payload(llava_observation)
    llava_clean_text = llava_visual_observation_text(
        llava_clean,
        raw_fallback=(
            llava_observation.get("llava_visual_observation")
            if isinstance(llava_observation, dict)
            else ""
        ),
    )
    raw_figure_refs = stable_unique(
        [
            *(doc.get("figure_refs") or []),
            *values_for_keys(doc, ("figure_refs", "figure_references", "figure_ref", "figure_reference", "figures")),
            *llava_figure_refs(llava_clean),
        ],
        limit=80,
    )
    raw_callouts = stable_unique(
        [
            *(doc.get("callouts") or []),
            *values_for_keys(doc, ("callouts", "callout", "visible_callouts", "labels")),
            *llava_callouts(llava_clean),
        ],
        limit=120,
    )
    raw_part_numbers = stable_unique(
        [
            *(doc.get("part_numbers") or []),
            *values_for_keys(doc, ("part_numbers", "part_number", "parts", "covered_part_number")),
        ],
        limit=120,
    )

    text_values = find_text_values(doc)
    figure_refs_from_text = figure_refs_from_texts(text_values)
    figure_refs_clean, structured_figure_metadata = clean_figure_refs([*raw_figure_refs, *figure_refs_from_text])
    callouts_clean = clean_callouts(raw_callouts)
    part_numbers = filter_real_part_numbers(
    ...
    if llava_observation:
        llava_text = compact_ws(llava_clean_text, limit=6000)
    ...
    subject = infer_subject(
        doc,
        source_summary + " " + llava_text,
        part_numbers,
        llava_subject=llava_subject_hint(llava_clean),
    )
    ...
    card: Dict[str, Any] = {
        ...
        "model_layers": {
            ...
            "llava_clean_payload_loaded": bool(llava_clean),
        },
        "retrieval_document": {
        ...
        "llava_clean_observation": llava_clean,
        "safety_contract": {
            "final_answer_allowed": False,
            "answer_permission": False,
...
def build(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    cards_path = output_dir / "trace_net_confirmed_image_page_summary_v1_2.jsonl"
    retrieval_path = output_dir / "trace_net_confirmed_image_page_summary_v1_2_retrieval_documents.jsonl"
    report_path = output_dir / "trace_net_confirmed_image_page_summary_v1_2_report.txt"
    ...
    llava_clean_payload_loaded_count = sum(bool((c.get("model_layers") or {}).get("llava_clean_payload_loaded")) for c in cards)
    ...
    summary = {
        ...
        "llava_clean_payload_loaded_count": llava_clean_payload_loaded_count,
    ...
    lines = [
        ...
        f"llava_clean_payload_loaded_count={llava_clean_payload_loaded_count}",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(f"llava_clean_payload_loaded_count={summary['llava_clean_payload_loaded_count']}")
