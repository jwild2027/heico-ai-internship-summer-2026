...
from scripts.operations.retrieval.run_trace_net_guided_candidate_discovery_v4 import (
    build_result,
    clean_nomenclature_value,
...
def test_v4_rejects_uuid_like_and_file_tokens():
    for token in [
        "248c-5c38-8683",
        "2485-5c0a-bfa1",
        "24d3241c-fc76-51e3-96dd",
        "247168638171824e-05",
        "00000024.tif",
        "00000124.tif",
    ]:
        quality, reason = classify_candidate_quality(token, f"part_number {token}", "ocr.txt")
        assert quality == "rejected_noise", (token, quality, reason)
        assert not is_probable_part_token(token)


def test_v4_keeps_real_strict_and_real_loose_candidates():
    q, reason = classify_candidate_quality("244CS-3-2", "part_number 244CS-3-2 AR - FASTENER", "ipl.csv")
    assert q == "valid_part_like"
    q2, _ = classify_candidate_quality("MS24693-C5", "part number MS24693-C5 TABLE, SNACK FOLDING, ASSY", "ipl.csv")
    assert q2 == "valid_part_like"
    q3, _ = classify_candidate_quality("120-48024-001", "part_number 120-48024-001 RING, LOCKING", "table.csv")
    assert q3 == "valid_part_like"


def test_v4_cleans_polluted_nomenclature_values():
    assert clean_nomenclature_value("HIGH_NAVIGATION_CONFIDENCE") is None
    assert clean_nomenclature_value("SOURCE_BOUND_RESPONSE_SHOULD_SUMMARIZE_SOURCE_LINKED_PAGE_ON") is None
    assert clean_nomenclature_value("HAS_TABLE_ROW") is None
    assert clean_nomenclature_value("MAINTENANCE MANUAL WITH ILLUSTRATED PARTS LIST AIRLINE EFF") is None
    assert clean_nomenclature_value("AR - FASTENER") == "AR - FASTENER"


def test_v4_collect_filters_uuid_tif_and_keeps_real_candidates(tmp_path: Path):
    root = tmp_path / "artifacts"
    root.mkdir()
    (root / "ocr.txt").write_text(
        "t_p_120_1176_p000180 part_number 248c-5c38-8683 fake id\n"
        "t_p_120_1176_p000024 file 00000024.tif fake file token\n"
        "t_p_120_1176_p000491 part_number 244CS-3-2 AR - FASTENER\n"
        "t_p_120_1176_p000055 part_number 120-48024-001 RING, LOCKING ATA 25-21-00\n",
        encoding="utf-8",
    )
    clues = parse_query_clues("part starts with 24")
    hits, evidence_count, rejected, weak = collect_evidence(root, clues)
    routes = merge_candidate_routes(hits)
    strict_parts = [r.candidate_part_number for r in routes if r.route_group == "strict_prefix"]
    loose_parts = [r.candidate_part_number for r in routes if r.route_group == "loose_contains"]
    assert "244CS-3-2" in strict_parts
    assert "120-48024-001" in loose_parts
    assert "248c-5c38-8683" not in strict_parts
    assert "00000024.tif" not in strict_parts + loose_parts
    assert rejected >= 2


def test_main_outputs_summary_with_noise_counts(tmp_path: Path):
    from scripts.operations.retrieval.run_trace_net_guided_candidate_discovery_v4 import main
