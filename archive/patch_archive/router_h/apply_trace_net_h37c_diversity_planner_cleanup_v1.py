...
NEW_EXTRACT_NOMENCLATURE_AND_HELPER = r"""
...
def _function_bounds(src: str, name: str) -> tuple[int, int]:
    ...
    # Find next top-level def/class after this function.
...
def main() -> int:
    ...
    backup = TARGET.with_suffix(TARGET.suffix + ".bak_h37c_diversity_planner_cleanup_" + datetime.now().strftime("%Y%m%d_%H%M%S"))
    ...
    try:
        ...
        # Replace _extract_nomenclature and insert helper immediately above it.
        src, ok = _replace_function(src, "_extract_nomenclature", NEW_EXTRACT_NOMENCLATURE_AND_HELPER)
        changed = changed or ok

        if "used_labels: set[str] = set()" not in src:
            anchor = "    per_route_idx: Counter[str] = Counter()\n"
            if anchor not in src:
                raise RuntimeError("failed_to_find_per_route_idx_anchor")
...
"""
        ...
        print("status=TRACE_NET_H37C_DIVERSITY_PLANNER_CLEANUP_FAILED_RESTORED")
        print(f"error={e}")
        print(f"backup={backup}")
        return 1

    print("status=TRACE_NET_H37C_DIVERSITY_PLANNER_CLEANUP_APPLIED")
