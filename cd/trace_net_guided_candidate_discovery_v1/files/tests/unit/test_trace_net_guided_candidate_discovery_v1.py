...
from scripts.operations.retrieval.run_trace_net_guided_candidate_discovery_v1 import (
...
def test_detect_partial_prefix_from_numbers_2_and_4():
    ...
    assert "full_part_number" in __import__("scripts.operations.retrieval.run_trace_net_guided_candidate_discovery_v1", fromlist=["missing_clues"]).missing_clues(clues)
...
def test_cli_writes_outputs(tmp_path):
    ...
    cmd = [
        sys.executable,
        "-B",
        "scripts/operations/retrieval/run_trace_net_guided_candidate_discovery_v1.py",
        "--artifact-root",
        str(root),
        "--output-dir",
        str(out),
