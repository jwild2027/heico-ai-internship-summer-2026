...
Retrieves relevant chunks from ChromaDB, assembles a context window,
calls an Ollama LLM to generate a grounded answer, and returns the
answer with source citations.

Also includes a test case runner for the seaplane handbook test suite.

Usage (CLI):
    python tools/langchain_adapter.py ask --query "What are sponsons?"
    python tools/langchain_adapter.py test
    python tools/langchain_adapter.py test --out results.json
...
DEFAULT_EMBED_MODEL     = base.DEFAULT_MODEL          # bge-large
DEFAULT_COLLECTION      = base.DEFAULT_COLLECTION     # pdf_chunks
DEFAULT_PERSIST_DIR     = base.DEFAULT_PERSIST_DIR    # chroma_db
DEFAULT_TOP_K           = 6
DEFAULT_FETCH_K         = 20
MAX_CONTEXT_WORDS       = 1500   # hard cap on context sent to LLM
DEFAULT_USE_HYDE        = False  # HyDE: embed a hypothetical answer instead of raw query (off by default, opt-in)
...
def generate_hypothetical_answer(query: str, llm_model: str) -> str:
    ...
    HyDE (Hypothetical Document Embeddings) improves retrieval when the
    user's query phrasing doesn't match the document's terminology.
    e.g. "How does a seaplane slow down?" → hypothetical answer mentions
    "no brakes", "wind", "current" → those terms match the actual chunk.
    """
    try:
        ...
        # Combine query + hypothetical so we don't lose the original intent
        return f"{query} {hyp}"
    except Exception:
        # Fall back to raw query if HyDE fails
        return query
...
def ask(
    ...
    *,
    ...
    Returns a dict with:
        ...
        citations   — list of {page, title, distance} dicts
        chunks      — raw retrieved chunk dicts
        latency_ms  — total latency
        grounded    — True if answer doesn't contain refusal language
        hyde_used   — True if HyDE was applied for retrieval
    """
    ...
    # This bridges the gap when user phrasing doesn't match document terminology
    hyde_used = False
    retrieval_text = query
    if use_hyde:
        hypothetical = generate_hypothetical_answer(query, llm_model)
        if hypothetical != query:  # HyDE succeeded
    ...
    # 2a. Parent expansion — for any child hit, swap in the full parent text.
    # This gives the LLM full surrounding context while keeping search precise.
    try:
    ...
    # 2b. Assemble context — for child hits, substitute the parent text
    # Dedupe so the same parent isn\'t added twice if two children both hit
    context_chunks = []
    ...
    grounded = not any(p in answer.lower() for p in refusal_phrases)
...
TEST_CASES = [
    # ============================================================
    # test-2.pdf — Seaplane handbook
    # ============================================================
    # Easy — exact terminology
    {
        "source_doc":     "test-2",
        "query":          "What are sponsons?",
        "expected_terms": ["sponson", "winglike", "hull", "stabilize"],
        "difficulty":     "easy",
    },
    {
        "source_doc":     "test-2",
        "query":          "What is the step on a float?",
        "expected_terms": ["step", "water drag", "takeoff", "longitudinal"],
        "difficulty":     "easy",
    },
    {
        "source_doc":     "test-2",
        "query":          "What are water rudders?",
        "expected_terms": ["water rudder", "retract", "maneuvering", "cables"],
        "difficulty":     "easy",
    },
    {
        "source_doc":     "test-2",
        "query":          "What is hydrodynamic lift?",
        "expected_terms": ["hydrodynamic", "upward force", "motion", "floats"],
        "difficulty":     "easy",
    },
    # Medium — concept present but phrased differently
    {
        "source_doc":     "test-2",
        "query":          "How does a seaplane slow down on water?",
        "expected_terms": ["brakes", "wind", "current", "motion"],
        "difficulty":     "medium",
    },
    {
        "source_doc":     "test-2",
        "query":          "What makes glassy water dangerous for landing?",
        "expected_terms": ["glassy", "altitude", "illusion", "featureless"],
        "difficulty":     "medium",
    },
    {
        "source_doc":     "test-2",
        "query":          "How do you know which side of a buoy to pass?",
        "expected_terms": ["red", "right", "returning", "channel", "nun", "can"],
        "difficulty":     "medium",
    },
    {
        "source_doc":     "test-2",
        "query":          "What causes a seaplane to weathervane?",
        "expected_terms": ["weathervane", "wind", "yaw", "nose"],
        "difficulty":     "medium",
    },
    # Hard — requires synthesizing across chunks
    {
        "source_doc":     "test-2",
        "query":          "What are the differences between flying boats and floatplanes?",
        "expected_terms": ["flying boat", "floatplane", "hull", "fuselage", "pontoon"],
        "difficulty":     "hard",
    },
    {
        "source_doc":     "test-2",
        "query":          "What regulations apply when a seaplane is on water?",
        "expected_terms": ["USCG", "vessel", "91.115", "right-of-way"],
        "difficulty":     "hard",
    },
    {
        "source_doc":     "test-2",
        "query":          "How does buoyancy work for seaplane floats?",
        "expected_terms": ["buoyancy", "displace", "weight", "fresh water"],
        "difficulty":     "hard",
    },
    # Adversarial — not in the seaplane document
    {
        "source_doc":     "test-2",
        "query":          "What is the engine horsepower requirement for a seaplane?",
        "expected_terms": [],
    ...
    {
        "source_doc":     "test-2",
        "query":          "How do you file a flight plan for a seaplane?",
        "expected_terms": [],
        "difficulty":     "adversarial",
        "expect_grounded": False,
    },
    ...
    # ============================================================
    # test-3.pdf — Remote Pilot sUAS Study Guide
    # ============================================================
    # Easy — exact terminology
    {
        "source_doc":     "test-3",
        "query":          "What is Class B airspace?",
        "expected_terms": ["class b", "10,000", "msl", "atc"],
        "difficulty":     "easy",
    },
    {
        "source_doc":     "test-3",
        "query":          "What does NOTAM stand for?",
        "expected_terms": ["notice", "airmen", "time-critical"],
        "difficulty":     "easy",
    },
    {
        "source_doc":     "test-3",
        "query":          "What frequency is used at a non-towered airport with no UNICOM or FSS?",
        "expected_terms": ["multicom", "122.9", "non-towered"],
        "difficulty":     "easy",
    },
    {
        "source_doc":     "test-3",
        "query":          "What is density altitude?",
        "expected_terms": ["density altitude", "standard", "air density"],
        "difficulty":     "easy",
    },
    # Medium — concept present, phrased differently
    {
        "source_doc":     "test-3",
        "query":          "What are the symptoms of hyperventilation?",
        "expected_terms": ["hyperventilation", "lightheaded", "tingling", "visual"],
        "difficulty":     "medium",
    },
    {
        "source_doc":     "test-3",
        "query":          "Why does humidity reduce drone performance?",
        "expected_terms": ["humidity", "density", "moist", "less dense"],
        "difficulty":     "medium",
    },
    {
        "source_doc":     "test-3",
        "query":          "What does the IMSAFE checklist cover?",
        "expected_terms": ["illness", "medication", "stress", "alcohol", "fatigue", "emotion"],
        "difficulty":     "medium",
    },
    {
        "source_doc":     "test-3",
        "query":          "How does excess weight affect a small unmanned aircraft?",
        "expected_terms": ["weight", "longer", "stalling", "takeoff", "landing"],
        "difficulty":     "medium",
    },
    # Hard — requires synthesizing across chunks
    {
        "source_doc":     "test-3",
        "query":          "What are the five hazardous attitudes a pilot can have?",
        "expected_terms": ["anti-authority", "impulsivity", "invulnerability", "macho", "resignation"],
        "difficulty":     "hard",
    },
    {
        "source_doc":     "test-3",
        "query":          "Describe the three stages of a thunderstorm's life cycle.",
        "expected_terms": ["cumulus", "mature", "dissipating", "updraft", "downdraft"],
        "difficulty":     "hard",
    },
    {
        "source_doc":     "test-3",
        "query":          "What is the difference between a restricted area and a prohibited area?",
        "expected_terms": ["prohibited", "restricted", "hazardous", "authorization"],
        "difficulty":     "hard",
    },
    # Adversarial — not in the sUAS document
    {
        "source_doc":     "test-3",
        "query":          "What is the maximum flight range of a DJI Mavic 3 in kilometers?",
        "expected_terms": [],
        "difficulty":     "adversarial",
        "expect_grounded": False,
    },
    {
        "source_doc":     "test-3",
        "query":          "How do I register my drone with EASA in the European Union?",
        "expected_terms": [],
        "difficulty":     "adversarial",
        "expect_grounded": False,
    },
...
def grade_result(case: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    # For adversarial cases, pass if NOT grounded (correctly refused)
...
def parse_args() -> argparse.Namespace:
    ...
    test_parser.add_argument("--out",         type=Path, help="Save results to JSON.")
    test_parser.add_argument("--no-answers",  action="store_true",
                            help="Hide answer text, show scores only.")
