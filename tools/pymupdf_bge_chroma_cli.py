...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest PDFs with PyMuPDF, embed with BGE, and query ChromaDB.")
    subparsers = parser.add_subparsers(dest="command", required=True)
    ingest_parser = subparsers.add_parser("ingest")
    ingest_parser.add_argument("--pdf", type=Path, required=True)
    ingest_parser.add_argument("--persist-dir", type=Path, default=DEFAULT_PERSIST_DIR)
    ingest_parser.add_argument("--collection", default=DEFAULT_COLLECTION)
    ingest_parser.add_argument("--model", default=DEFAULT_MODEL)
...
def ocr_page_text(page: Any, dpi: int = OCR_DPI) -> str:
    try:
        ...
        raise ModuleNotFoundError("OCR fallback needs pytesseract and Pillow.") from error
...
def build_chunks(pages: list[dict[str, Any]], target_words: int, max_words: int,
                 overlap_blocks: int, source_name: str) -> list[ChunkRecord]:
    ...
    def flush_chunk() -> None:
        ...
        metadata = {
            "source": source_name, "page_start": page_start, "page_end": page_end,
            "section_title": title, "word_count": block_word_count(body_text),
            "block_count": len(current_blocks),
        }
        chunks.append(ChunkRecord(
            chunk_id=f"{source_name}:chunk-{chunk_number:04d}",
            title=title, text=body_text, metadata=metadata,
        ))
        chunk_number += 1
        carry_blocks = [strip_heading_from_block(b) for b in current_blocks[-overlap_blocks:]] if overlap_blocks > 0 else []
...
def embed_texts(model: str, texts: list[str], *, kind: str = "passage", show_progress: bool = False) -> list[list[float]]:
    ...
    for idx, text in enumerate(texts, start=1):
        ...
        if len(windows) == 1:
            try:
                safe_w = " ".join(windows[0].split()[:100])[:400]
        ...
        try:
            safe_windows = [" ".join(w.split()[:100])[:400] for w in windows]
            response = ollama.embed(model=model, input=safe_windows, truncate=True)
...
def ingest_pdf(pdf_path: Path, persist_dir: Path, collection_name: str, model: str, chunk_words: int, overlap: int) -> None:
    ...
    chunks = build_chunks(pages, target_words=chunk_words, max_words=max(chunk_words, DEFAULT_MAX_WORDS),
                          overlap_blocks=DEFAULT_OVERLAP_BLOCKS, source_name=pdf_path.stem)
    if not chunks:
...
def lexical_rerank(query: str, results: dict[str, list[list[Any]]], top_k: int) -> list[tuple[str, dict[str, Any], float | None, int]]:
    ...
    # FIX 3: Detect NIST control IDs in query (e.g. AC-2, IA-2, SC-8)
    # Boost chunks whose section_title contains the exact control ID
    ...
    for document, metadata, distance in zip(documents, metadatas, distances):
        ...
        # Strong boost when chunk title contains the exact control ID being asked about
        if control_ids:
            title = (metadata or {}).get("section_title", "").upper()
            for cid in control_ids:
                if cid in title:
                    overlap_score += 10  # dominates lexical score
...
def cross_encoder_rerank(query: str, results: dict[str, list[list[Any]]], model: str, top_k: int) -> list[tuple[str, dict[str, Any], float | None, float]]:
    ...
    try:
        ...
        print("[info] 'sentence-transformers' is not installed. Falling back to lexical rerank.")
        lex = lexical_rerank(query, results, top_k)
        return [(doc, meta, dist, float(score), "lexical") for (doc, meta, dist, score) in lex]
    except Exception as e:
...
def query_collection(persist_dir: Path, collection_name: str, model: str, query: str,
                     top_k: int, fetch_k: int, rerank: bool = False, rerank_model: str | None = None) -> None:
    collection = get_collection(persist_dir, collection_name)
...
def main() -> None:
    ...
    if args.command == "query":
        query_collection(persist_dir=args.persist_dir, collection_name=args.collection,
                         model=args.model, query=args.query, top_k=args.top_k,
                         fetch_k=DEFAULT_FETCH_K, rerank=args.rerank,
